ifdef(`PERL',,#! /bin/awk -f
)#
define(VERSION,`2.4.3')#
define(VERDATE,` 15 Jul 2005')#
changecom()#
######################################################################
#
ifdef(`PERL',`#'
...
###########################################################################
define(`dummy',#
#  If you are reading the m4 script, at this point you will find the macro
#  definitions used for the build.  If you modify this program, please note
#  the following:
#
...
#  Defining "NOLOWER" replaces calls to the built-in "tolower" or "toupper"
#    with loops
...
changequote(\036,\037)#
ifdef(\036PERL\037,#
  \036define\037(\036NOLOWER\037,1)#
  \036define\037(\036BADSPLIT\037,1)#
  \036define\037(\036NOFUNCS\037,1),)#
define(dosplit,ifdef(\036BADSPLIT\037,
{ $1 = \036\036split\037\037($2,$3,$4)
  if ($1 >= 1 && $3[$1] == "" && $4 == " ") --$1
},$1 = \036\036split\037\037($2,$3,$4)))#
define(squeezname,#
#
...
#
  ...
  for( $6=1; $6 <= $3; ++$6) {
    ...
    if( $5 == " " || \
        ...
        $5 == "*" || \
        $5 == "/" ) {
        ifdef(\036NOLOWER\037,mychar = substr((UCAZ mychar),\
          index((lcaz mychar), mychar),1),mychar = toupper(mychar))
    }
  ...
  #
  \036dotoupper\037(name_temp,$4)
  if (rep_name[name_temp] != "") $4 = rep_name[name_temp]
  #
  ...
  if (index($4,comma) != 0 ){
    ...
    nam_suf = ""
    \036dosplit\037(num_namp,$4,x_namep," ")
    if (num_namp > 1) {
      \036dotoupper\037(xtemp,x_namep[num_namp])
      if (rep_suffix[xtemp] != ""){
        if(junior_on_last == "yes") {
    ...
    for (kc=llname-1; kc>1; --kc) {
      ...
      if (cc == ".") {
        if (cp != " ") {
          $4 = (substr($4,kc+1,llname-kc) comma " " \
            substr($4,1,kc))
        } else {
          $4 = (substr($4,kc+2,llname-kc-1) comma " " \
            substr($4,1,kc))
        } # if (cp != " ")
        kc=0
      } # end if (cc == ".")
...
define(seq_comp,
  ...
  while ( $7 <= $4) {
    ...
    while ( $6 > 0 && sm == 0 ) {
      cr = substr($1, $7, $6)
      sm = index(substr($2,$8,$5-$8+1),cr)
      sf = 1
      if (!(known_links == "") && !(sm == 0) && ($6 < seqlim) ) {
        if ($7 < $4 && ipass > 0) {
          kl = substr(known_links, $7+$6-1, 1 )
          if (kl == "1" && ipass < 3 && $3[$7+$6] > 0 ) {
            if( !(sm+$8+$6-1 == $3[$7+$6]) && (ipass < 3) ) {
              sm = 0
            }
          } else {
            kbase = $7+$6-1
            kbreak = 0
            while ( (kbase < $4) && !($3[kbase+1] > 0) ) { 
              kbase++
              if (substr(known_links, kbase, 1) == "0") kbreak++
            }
            if ( ($3[kbase+1] > 0) && \
              (((sm+$8-1+kbase-$7-$6 > $3[kbase+1] + kbreak*3 -1 )) ||\
               (sm+$8-1+kbase-$7-$6 < $3[kbase+1] - kbreak*3 -2 - seqlim*2 )) ) {
               sf = -1
            }
          }
        }
        if ($7 > 1 && !( sm == 0) && sf == 1 ) {
          kl = substr(known_links, $7, 1 )
          if ( kl == "1" && ipass < 3 && $3[$7-1] > 0 ) {
            if( !(sm+$8-2 == $3[$7-1]) && (ipass < 3) ) {
              sm = 0
            }
          } else {
            kbase = $7
            kbreak = 0
            while ( ((kbase-1) > 0)  && !($3[kbase] > 0) ) { 
              kbase--
              if (substr(known_links, kbase, 1) == "0") kbreak++
            }
            if ( ($3[kbase] > 0)  && \
              (((sm+$8-1+kbase-$7 < $3[kbase]  - kbreak*3 - 1 ) ) || \
               sm+$8-1+kbase-$7 > $3[kbase]  + kbreak*3 + seqlim*2-1) ) {
               sf = -1
            }
          }
        }
      ...
      if (sm > 0) {
        kbase = $7
        while ( ( kbase > 1)  && ($3[kbase-1] <= 0) && (sf > 0) && \
          (substr($1, kbase-1, 1) == substr($2, sm+$8-2+kbase-$7, 1) )){
            kbase--
            nm++
            $3[kbase] = sm+$8-1+kbase-$7
        }
        for ($9=0; $9 < length(cr); ++$9) {
          if ($3[$7] == 0 ) {
            $3[$7] = (sm+$9+$8-1)*sf
            ++$7
          } else {
        ...
        $8 = sm+length(cr)+$8-1
        if (length(cr) > 3 || $7 == $4+1) {
          if (sf > 0) nm += length(cr)
        }
        break
      } else {
        if ($6 > 16) {
  ...
  qxmaxlen = $4
  if ($5 > $4) qxmaxlen = $5
  xreturn(100*(nm/qxmaxlen))
)#
define(dotolower,ifdef(\036NOLOWER\037,{
  lx_tl = length($2)
  tx_tl = $2
  $1 = ""
...
define(dotoupper,ifdef(\036NOLOWER\037,{
  lx_tu = length($2)
  tx_tu = $2
  $1 = ""
...
define(xreturn,ifdef(\036NOFUNCS\037,ret_val=$1,return($1))
)#
define(dosqueez,ifdef(\036NOFUNCS\037,
{\036\036squeezname\037\037($2,temp_a_name)
$1=ret_val},
$1=\
  \036\036\036squeezname\037\037\037($2)))#
define(dosqueezt,ifdef(\036NOFUNCS\037,
{\036\036squeeztype\037\037($2,temp_a_type)
$1=ret_val},
$1=\
  \036\036\036squeeztype\037\037\037($2)))#
define(dotypeset,ifdef(\036NOFUNCS\037,
{\036\036typeset\037\037($2,lostr,lstr,mystr,pchar,qtsi)
$1=ret_val},
$1=\
  \036\036\036typeset\037\037\037($2))
)#
define(donameset,ifdef(\036NOFUNCS\037,
{\036\036nameset\037\037($2,lostr,lstr,mystr,pchar,qnsi)
$1=ret_val},
$1=\
  \036\036\036nameset\037\037\037($2))
)#
define(doseq_comp,ifdef(\036NOFUNCS\037,
{\036\036seq_comp\037\037($2,$3,$4,lsal,lssr,lm,isha,isr,ksa,nm)
$1=ret_val},
$1=\
  \036\036\036seq_comp\037\037\037($2,$3,$4))
)##############################################################################
#
# Version History:
...
#       "no" to suppress them
ifdef(\036USEPREFIX\037,,\036define(\036USEPREFIX\037,yes)\037)#
        use_pdb2cif_prefix = "USEPREFIX()"
#
#
...
#       parsing from that point onwards
        ...
        if( use_pdb2cif_prefix == "no" ) pdb2cif_prefix = ""
        record_number = " "
        remark_flag = 0
...
#
        dosplit(num_ctypes, \
          ". . . . hydrog hydrog saltbr hydrog hydrog saltbr", \
          connect_types," ") 
#
...
#
        for (yy=0; yy < 100; ++yy) {
        	yyyy[yy+0] = yy+1900
        	if ( yy < 70 ) yyyy[yy+0] += 100
        }
#
#       Define lists of amino acids and nucleic acids
        dosplit(num_aa,("ABU ACD ALA ALB ALI ARG ARO ASN ASP ASX" \
                        " BAS CYS GLN GLU GLX GLY HIS HYP ILE LEU" \
                        " LYS MET PCA PHE PRO SER THR TRP TYR VAL"),\
                        aa_list, " ")
        dosplit(num_na,("A +A C +C G +G I +I T +T U +U"),na_list, " ")
...
"Formic Acid;FOR;C1 H2 O2;40.03|" \
"Water;HOH;H2 O1;18.015"),
  AARESES_list,"|")

for (naa = 1; naa <= num_AARESES; ++naa){
  dosplit(nxx, AARESES_list[naa], naa_split, ";")
...
for (nna = 1; nna <= num_NARESES; ++nna){
  dosplit(nxx, NARESES_list[nna], nna_split, ";")
...
#
        dosplit(num_per_tab,( " . D  " \
         ...
         "         DB JL RF BH HN MT"),\
         periodic_table, " ")
         dosplit(num_aa_na_el,(". C H D N O P S"), \
           standard_res_elements," ")
         dosplit(num_one_let_el,(". B C H D N O F P S K V Y I W"), \
           one_letter_elements," ")
...
{ if (end_flag == 0) {
  xlat_save = xlat_flag
  non_xlated[++xlat_flag] = $0
  first_field = $1
  if (NF > 1 && ($1 == "#def" || $1 == "#define")) {
    dotolower(var_name,$2)
    var_value = ""
    if (NF > 2) dotolower(var_value,$3)
    if (var_name == "verbose" && \
      (var_value == "yes" || var_value == "no")) {
      verbose = var_value
      xlat_flag = xlat_save
    } else {
      if (var_name == "convtext" && \
        (var_value == "yes" || var_value == "no")) {
        convtext = var_value
        xlat_flag = xlat_save
      } else {
        if (var_name == "auth_convtext" &&
          (var_value == "yes" || var_value == "no" || \
          var_value == "conditional")) {
          auth_convtext = var_value
          xlat_flag = xlat_save
        } else {
          if (var_name == "dense_list" &&
            (var_value == "yes" || var_value == "no")) {
            dense_list = var_value
            xlat_flag = xlat_save
          } else {
            if (var_name == "junior_on_last" &&
              (var_value == "yes" || var_value == "no")) {
              junior_on_last = var_value
              xlat_flag = xlat_save
            } else {
              if (var_name == "print_ter" &&
                (var_value == "yes" || var_value == "no" || \
                var_value == "comment")) {
                print_ter = var_value
                xlat_flag = xlat_save
              } else {
                if (var_name == "compliance_level") {
                  compliance_level = var_value
                  xlat_flag = xlat_save
                } else {
                  if (var_name == "use_pdb2cif_prefix") {
                    if (var_value == "no") {
                    } else {
                      if (var_value == "yes") {
    ...
    if (NF > 3 && var_name == "name") {
      ...
      dotypeset(PDB_form,PDB_form)
      dotoupper(PDB_form,PDB_form)
  ...
  if (length($0) > 5) {
    ...
    if (length(first_field) > 6 ) first_field = substr($1,1,6)
    if (first_field != previous_keyword && flag != "0") {
      printf ("; \n\n")
      flag = "0"
...
{
  if (first_field == "ATOM" || first_field == "HETATM" \
    || first_field == "TER") {
    xlat_flag=xlat_save
  ...
  #
  atom_pdb                            = substr( ($0),  7, 5)
  atom_number[atom_flag]              = substr( ($0),  7, 5)
  if ( atom_point[0+atom_number[atom_flag]] == "" ) {
    atom_point[0+atom_number[atom_flag]]  = atom_flag
  } else {
    atom_unique_ids = "no"
  }
  atom_name[atom_flag]                = substr( ($0), 13, 4)
  dosqueez(temp_name,atom_name[atom_flag])
  atom_name[atom_flag]                = temp_name
  residue_name[atom_flag]             = substr( ($0), 18, 3)
  temp_name = substr( temp_name, 1, 2)
  dotoupper(temp_name,temp_name)
  ...
  if (substr (xtemp_name, 1, 1) == " ") \
    xtemp_name = ytemp_name
  found = "false"
  if (first_field == "ATOM") {
    ++atom_res_flag
    if (res_count[residue_name[atom_flag]] == "") {
      ...
      warning_list[++warning_flag] = \
        ("#=# ATOM_SITE: Residue name " \
        residue_name[atom_flag]" not in standard residue list \n")
    }
    ++res_count[residue_name[atom_flag]]
    for (ii = 1; ii <= num_aa_na_el && found == "false"; ++ii) {
    if (xtemp_name == standard_res_elements[ii]) found = "true"
    }
  } else {
    if (index(xhet_formula[residue_name[atom_flag]],xtemp_name) >0){
      found = "true"
    } else {
      if (index(xhet_formula[residue_name[atom_flag]], \
        ...
        warning_list[++warning_flag] = \
        ("#=# ATOM_SITE: Het group " \
            residue_name[atom_flag]"; atom type " xtemp_name \
        " converted to " ytemp_name "\n")
        ...
        for (ii = 1; ii <= num_per_tab && found == "false"; ++ii) {
        if (xtemp_name == periodic_table[ii]) found = "true"
        }
      }
    }
  }
  if (found == "false") {
    ...
    if (first_field == "ATOM") {
      for (ii = 1; ii <= num_aa_na_el && found == "false"; ++ii) {
      if (ytemp_name == standard_res_elements[ii]) found = "true"
      }
    } else {
      for (ii = 1; ii <= num_one_let_el && found == "false"; ++ii) {
      if (ytemp_name == one_letter_elements[ii]) found = "true"
      }
    }
    if ( found == "true") {
      temp_name  = ( " " ytemp_name )
    ...
    warning_list[++warning_flag] = \
      ("#=# ATOM_SITE: Site " atom_pdb"; unexpected atom type " xtemp_name \
      " converted to " temp_name "\n")
  }
  atom_type[atom_flag]                = substr( temp_name, 1, 2)
  atom_alt_location[atom_flag]        = substr( ($0), 17, 1)
  chain_id[atom_flag]                 = substr( ($0), 22, 1)
  residue_seq_number[atom_flag]       = substr( ($0), 23, 5)
  atom_x[atom_flag]                   = substr( ($0), 31, 8)
  atom_y[atom_flag]                   = substr( ($0), 39, 8)
  atom_z[atom_flag]                   = substr( ($0), 47, 8)
  atom_occ[atom_flag]                 = substr( ($0), 55, 6)
  B_or_U[atom_flag]                   = substr( ($0), 61, 6)
  footnote_number[atom_flag]          = substr( ($0), 68, 3)
  if (compliance_level >= 2.0) {
    atom_seg_id[atom_flag]            = substr( ($0), 73, 4)
    atom_type[atom_flag]              = substr( ($0), 77, 4)
  }
  dosqueezt(temp_type,atom_type[atom_flag])
  ...
  if (model_flag != ".") {
    ...
    if (model_asn_low[model_count] == model_asn_high[model_count]) {
  ...
  #
  if (first_field == "ATOM")	atom_or_het[atom_flag] = "ATOM"
  if (first_field == "HETATM")	atom_or_het[atom_flag] = "HETATM"
  if (first_field == "TER")        atom_or_het[atom_flag] = "TER"
  #
  #   set alternate location value if blank
  #
  if (atom_alt_location[atom_flag] == " ") \
  ...
  #
  if (first_field == "HETATM" && chain_id[atom_flag] == " ")    {
    chain_id[atom_flag] = "."
    if (num_poly_ents == 1 && entities[1] == bcid) {
      chain_id[atom_flag] = bcid
    }
  }
  ...
  #
  if (first_field == "ATOM" && chain_id[atom_flag] != " ")    {
    entity_id[atom_flag] = chain_id[atom_flag]   }
  if (first_field == "TER"  && chain_id[atom_flag] != " ")    {
    entity_id[atom_flag] = chain_id[atom_flag]   }

  #
  #  set _entity.id to residue_name for HETATM records
  #
  if (first_field == "HETATM")   {
    ...
    if (entity_seq_num[residue_name[atom_flag]] != "" &&\
...

{
        ...
        a_atom_serial_number[aniso_flag] = substr( ($0),  7, 5)
        aniso_point[(a_atom_serial_number[aniso_flag] "|" model_flag)] = \
                                         aniso_flag
        atom_U11[aniso_flag]             = substr( ($0), 29, 7)
        atom_U22[aniso_flag]             = substr( ($0), 36, 7)
        atom_U33[aniso_flag]             = substr( ($0), 43, 7)
        atom_U12[aniso_flag]             = substr( ($0), 50, 7)
        atom_U13[aniso_flag]             = substr( ($0), 57, 7)
        atom_U23[aniso_flag]             = substr( ($0), 64, 7)
...

{
  if (first_field == "AUTHOR") {
    ...
    dosplit(num_auth, text, authors, comma )
    for (i=1; i <= num_auth; ++i){
      dosplit(num_a_split, authors[i], a_split, " ")
      authors[i] = ""
      if (num_a_split > 0) {
        authors[i] = a_split[1]
      ...
      if (auth_convtext == "yes" || \
        (auth_convtext == "conditional" && convtext == "yes")) {
        donameset(authors[i],authors[i])
      } 
    }
    is_blank = authors[num_auth]
    if (is_blank == "") --num_auth

    if (num_auth >= 1 && author_flag == "1") {
      printf ("\n\n\n")
      printf ("####################\n")
      printf ("#                  #\n")
      printf ("# AUDIT_AUTHOR     #\n")
...
{
        if (first_field == "CAVEAT") {
        xlat_flag=xlat_save

          caveat_cont           = substr( ($0),  9,  2)
          caveat_id             = substr( ($0), 12,  4)
          caveat_text           = substr( ($0), 20, 51)
...
{
  if (first_field == "CISPEP") {
        xlat_flag=xlat_save

    cp_res_name_end[++cispep_flag]        = substr( ($0),26, 3)
    cp_chain_id_end[cispep_flag]          = substr( ($0),30, 1)
    cp_res_seq_num_end[cispep_flag]       = substr( ($0),32, 5)
    cp_modnum[cispep_flag]                = substr( ($0),44, 3)
    cp_omega[cispep_flag]                 = substr( ($0),54, 6)


    if (cp_res_name_end[cispep_flag] == "   ") \
      cp_res_name_end[cispep_flag] = " . "
    if (cp_chain_id_end[cispep_flag] == " ") \
      cp_chain_id_end[cispep_flag] = bcid
    if (cp_modnum[cispep_flag] == "   ") {
       cp_modnum[cispep_flag] = " . "
    }
    if (cp_omega[cispep_flag] == "      ") {
...
#
{
        if (first_field == "COMPND") {
          xlat_flag=xlat_save
          compnd_contin        = substr( ($0),  9,  2)
          if (compnd_contin == "  ") \
            compnd[compnd_flag++] = "Compound::"
          bp = "     "
          if (compnd_contin != "  ") bp = "    "
          compnd[compnd_flag]  = (bp substr( ($0), 11, 60))
          # typeset information, if requested
          if (convtext == "yes") \
             dotypeset(compnd[compnd_flag],compnd[compnd_flag] )
        	++compnd_flag
                             }
}
#======================================================================
...
#
{
        if (first_field == "CONECT") {
        xlat_flag=xlat_save
        connect_save[++connect_flag] = substr( ($0), 1, 61)
...
{
      if (first_field == "DBREF") {
        xlat_flag=xlat_save
        dbref_chainID[++dbref_flag]   = substr( ($0), 13,  1)
        if (dbref_chainID[dbref_flag] == " ") \
            dbref_chainID[dbref_flag] = bcid
        dbref_seqBegin[dbref_flag]    = substr( ($0), 15,  5)
        dbref_seqEnd[dbref_flag]      = substr( ($0), 21,  5)
        dbref_database[dbref_flag]    = substr( ($0), 27,  6)
        dbref_dbAccession[dbref_flag] = substr( ($0), 34,  8)
        dbref_dbIdCode[dbref_flag]    = substr( ($0), 43, 12)
        dbref_dbseqBeg[dbref_flag]    = substr( ($0), 56,  6)
        dbref_dbseqEnd[dbref_flag]    = substr( ($0), 63,  6)
        dosplit(numx,dbref_database[dbref_flag],dblist," ")
        dbref_database[dbref_flag] = "."
        if (numx > 0) {
          dbref_database[dbref_flag] = dblist[1]
          for (j=2; j <= numx; ++j) {
            dbref_database[dbref_flag] = \
              ( dbref_database[dbref_flag] "_" dblist[j] )
          }
        }
        dosplit(numx,dbref_dbAccession[dbref_flag],dblist," ")
        ...
        dosplit(numx,dbref_dbIdCode[dbref_flag],dblist," ")
        dbref_dbIdCode[dbref_flag] = ""
        if (numx > 0) {
          dbref_dbIdCode[dbref_flag] = (" "dblist[1])
          for (j=2; j <= numx; ++j) {
            dbref_dbIdCode[dbref_flag] = \
              ( dbref_dbIdCode[dbref_flag] "_" dblist[j] )
          }
        }
        if((" " dbref_dbAccession[dbref_flag]) == \
...
#
{
        if (first_field == "EXPDTA") {
        xlat_flag=xlat_save

          # parse field

        expdta         = substr( ($0),  11, 60)
        nmr_flag       = index(expdta,"NMR")
        dotolower(loexpdta,expdta)
        dosplit(num_expdta,loexpdta, exp_split, " ")
                  loexpdta = ""
                  if (num_expdta > 0) {
...
{
  if (first_field == "FORMUL") {
  xlat_flag=xlat_save

  # parse field

  formul_het_number[formul_flag]      = substr( ($0),  9, 2)+0
  formul_het_site_symbol[formul_flag] = substr( ($0), 13, 3)
  hetatm_entity = formul_het_site_symbol[formul_flag]
  formul_het_cont_flag[formul_flag]   = substr( ($0), 17, 2)
  hetatm_entity = substr( ($0), 13, 3)
  entity_seq_num[hetatm_entity] = \
    formul_het_number[formul_flag]+0
  formul_het_text[formul_flag]        = substr( ($0), 20,51)
  if ( substr( ($0), 17, 2) == "  " ) {
  ...
  pxxc = ""
  for (ii = 0; ii <= 50; ++ii) {
    xxc = substr( ($0), 20+ii, 1)
    if ( index(UCAZ, xxc) == 0) xxc = " "
    if (pxxc != " " || xxc != " ") \
      xhet_formula[hetatm_entity] = \
      (xhet_formula[hetatm_entity] xxc)
    pxxc = xxc
  }
...
#   footnote text    == _atom_sites_footnote.text
{
        ...
        x = substr ( ($0), 10, 1)
        	...
        	ftnote_num   =  substr ( ($0), 8, 3) + 0
                ftnote_text  =  substr ( ($0), 12, 59)
...
#                                       _audit.revision_id
{
        ...
        head_funct_class        =       substr ( ($0), 11, 40)
        head_dep_date           =       substr ( ($0), 51,  9)
        head_PDB_code           =       substr ( ($0), 63,  4)
        if (head_PDB_code == "    ") {
          head_PDB_code = "."
        }
#
...
#
{
        if (first_field == "HELIX") {
        xlat_flag=xlat_save

        helix_no[ss_flag]                   = substr( ($0), 8, 3)
        helix_id[ss_flag]                   = substr( ($0),12, 3)
        helix_res_name_beg[ss_flag]         = substr( ($0),16, 3)
        helix_chain_id_beg[ss_flag]	    = substr( ($0),20, 1)
        helix_res_seq_beg[ss_flag]          = substr( ($0),22, 5)
        helix_res_name_end[ss_flag]         = substr( ($0),28, 3)
        helix_chain_id_end[ss_flag]         = substr( ($0),32, 1)
        helix_res_seq_end[ss_flag]          = substr( ($0),34, 5)
        helix_class[ss_flag]                = substr( ($0),39, 2)
        helix_comment[ss_flag]              = substr( ($0),41,30)
        if (helix_comment[ss_flag] == "                              " || \
...
#
{
        ...
        het_site_symbol[het_flag]       = substr( ($0),  8, 3)
        het_site_chain[het_flag]        = substr( ($0), 13, 1)
        het_site_residue[het_flag]      = substr( ($0), 14, 5)
        het_atoms_number[het_flag]      = substr( ($0), 21, 4)
        het_site_text[het_site_symbol[het_flag]] \
                                        = substr( ($0), 31,40)

  if ( het_site_chain[het_flag] == " ") {
...
#
{
        if (first_field == "HETNAM") {
        xlat_flag=xlat_save

          # parse field

        hetnam_cont[hetnam_flag]        = substr( ($0),  9, 2)
        hetnam_symbol[hetnam_flag]      = substr( ($0), 12, 3)
        hetnam_text[hetnam_flag]        = substr( ($0), 16,55)
        if( hetnam_cont[hetnam_flag] == "  "){
...
#
{
        if (first_field == "HETSYN") {
        xlat_flag=xlat_save

          # parse field

        hetsyn_cont[hetsyn_flag]        = substr( ($0),  9, 2)
        hetsyn_symbol[hetsyn_flag]      = substr( ($0), 12, 3)
        hetsyn_text[hetsyn_flag  ]      = substr( ($0), 16,55)
        if( hetsyn_cont[hetsyn_flag] == "  ") {
...
#
{
        if (first_field == "HYDBND") {
        xlat_flag=xlat_save

        hb_atom_beg[++hydbnd_flag]              = substr( ($0),13, 4)
        hb_alt_loc_beg[hydbnd_flag]             = substr( ($0),17, 1)
        hb_res_name_beg[hydbnd_flag]            = substr( ($0),18, 3)
        hb_chain_id_beg[hydbnd_flag]            = substr( ($0),22, 1)
        hb_res_seq_num_beg[hydbnd_flag]         = substr( ($0),24, 5)
        hb_atom_ha[hydbnd_flag]                 = substr( ($0),30, 4)
        hb_alt_loc_ha[hydbnd_flag]              = substr( ($0),34, 1)
        hb_chain_id_ha[hydbnd_flag]             = substr( ($0),36, 1)
        hb_res_seq_num_ha[hydbnd_flag]          = substr( ($0),38, 5)
        hb_atom_end[hydbnd_flag]                = substr( ($0),44, 4)
        hb_alt_loc_end[hydbnd_flag]             = substr( ($0),48, 1)
        hb_res_name_end[hydbnd_flag]            = substr( ($0),49, 3)
        hb_chain_id_end[hydbnd_flag]            = substr( ($0),53, 1)
        hb_res_seq_num_end[hydbnd_flag]         = substr( ($0),55, 5)
        hb_symm_1[hydbnd_flag]                  = substr( ($0),60, 6)
        hb_symm_2[hydbnd_flag]                  = substr( ($0),67, 6)


        dosqueez(temp_name,hb_atom_beg[hydbnd_flag])
        hb_atom_beg[hydbnd_flag] = temp_name
        dosqueez(temp_name,hb_atom_end[hydbnd_flag])
        hb_atom_end[hydbnd_flag] = temp_name
        dosqueez(temp_name,hb_atom_ha[hydbnd_flag])
        hb_atom_ha[hydbnd_flag] = temp_name
        if (hb_alt_loc_beg[hydbnd_flag] == " ") \
          hb_alt_loc_beg[hydbnd_flag] = "."
        if (hb_alt_loc_end[hydbnd_flag] == " ") \
          hb_alt_loc_end[hydbnd_flag] = "."
        if (hb_alt_loc_ha[hydbnd_flag] == " ") \
          hb_alt_loc_ha[hydbnd_flag] = "."
        if (hb_res_name_beg[hydbnd_flag] == "   ") \
          hb_res_name_beg[hydbnd_flag] = " . "
        if (hb_res_name_end[hydbnd_flag] == "   ") \
          hb_res_name_end[hydbnd_flag] = " . "
        if (hb_res_seq_num_beg[hydbnd_flag] == "     ") \
          hb_res_seq_num_beg[hydbnd_flag] = "  .  "
        if (hb_res_seq_num_end[hydbnd_flag] == "     ") \
          hb_res_seq_num_end[hydbnd_flag] = "  .  "
        if (hb_res_seq_num_ha[hydbnd_flag] == "     ") \
          hb_res_seq_num_ha[hydbnd_flag] = "  .  "
        if (hb_chain_id_beg[hydbnd_flag] == " ") \
          hb_chain_id_beg[hydbnd_flag] = bcid
        if (hb_chain_id_end[hydbnd_flag] == " ") \
          hb_chain_id_end[hydbnd_flag] = bcid
        if (hb_chain_id_ha[hydbnd_flag] == " ") \
          hb_chain_id_ha[hydbnd_flag] = bcid
        if (hb_symm_1[hydbnd_flag] == "      ") {
          hb_symm_1[hydbnd_flag] = "   .   "
        } else {
          hb_symm_1[hydbnd_flag] = \
            (substr(hb_symm_1[hydbnd_flag],1,3) "_" \
            substr(hb_symm_1[hydbnd_flag],4,3))
        }
        if (hb_symm_2[hydbnd_flag] == "      ") {
          hb_symm_2[hydbnd_flag] = "   .   "
        } else {
          hb_symm_2[hydbnd_flag] = \
            (substr(hb_symm_2[hydbnd_flag],1,3) "_" \
            substr(hb_symm_2[hydbnd_flag],4,3))
        }
...
#                                =  _citation.details
{
        if (first_field == "JRNL") {
          ...
          flush_refs = 1
                ...
                jrnl_rec_type   = substr( ($0), 13, 4)
                jrnl_cont       = substr( ($0), 17, 2)
                jrnl_title      = substr( ($0), 20, 51)
                text            = substr( ($0), 20, 51)
                jrnl_auth       = text
                if (convtext == "yes") \
                  dotypeset(text,text)
        ...
        if (jrnl_rec_type == "TITL" && jrnl_cont == "  ") {
           cit_title_1[cit_flag] = jrnl_title
           cit_title_2[cit_flag] = ""
        }
        if (jrnl_rec_type == "TITL" && jrnl_cont != "  ") {
           if (cit_title_2[cit_flag] == "") cit_title_2[cit_flag] = jrnl_title
           else \
           cit_title_2[cit_flag] = (cit_title_2[cit_flag] "\n  " jrnl_title)
           }

        if (jrnl_rec_type == "AUTH" && jrnl_cont == "  ") {
        ...
        if (jrnl_rec_type == "REF " && jrnl_cont == "  ") {
        	jour_1[cit_flag]    = substr ( ($0), 20, 28 )
        	jour_2[cit_flag]    = ""
            volu[cit_flag]      = substr ( ($0), 52, 4)
        	page[cit_flag]      = substr ( ($0), 57, 5)
        	year[cit_flag]      = substr ( ($0), 63, 4)
            jrnl_pub_pub_1[cit_flag]    = "?"
         }
        if (jrnl_rec_type == "REF " && jrnl_cont != "  ") {
            if ( jour_2[cit_flag] == "") jour_2[cit_flag] = substr ( ($0), 20, 28)
            else \
            jour_2[cit_flag]    = \
              (jour_2[cit_flag] "\n  " substr ( ($0), 20, 28) ) 
        }
        if (jrnl_rec_type == "PUBL" && jrnl_cont == "  " ) {
        jrnl_pub_pub[cit_flag]      = substr ( ($0), 20, 51 )
                jour_1[cit_flag]    = "?"
                if (volu[cit_flag] == "" || \
                  volu[cit_flag] == "    " ) \
                volu[cit_flag]      = "?"
        ...
        if (jrnl_rec_type == "PUBL"  && jrnl_cont == "  ") {
             jrnl_pub_pub_1[cit_flag] = substr ( ($0), 20, 51 )
             jrnl_pub_pub_2[cit_flag] = ""
        }
        if (jrnl_rec_type == "PUBL"  && jrnl_cont != "  ") {
             if ( jrnl_pub_pub_2[cit_flag] == "") \
               jrnl_pub_pub_2[cit_flag] = substr ( ($0), 20, 51)
             else \
             jrnl_pub_pub_2[cit_flag] = \
               (jrnl_pub_pub_2[cit_flag] "\n  " substr ( ($0), 20, 51) ) 
        }
        if (jrnl_rec_type == "REFN") {
        	astm[cit_flag]      = substr ( ($0), 25, 6)
        	country[cit_flag]   = substr ( ($0), 33, 2)
                if (country[cit_flag] == "  ") {
                   country[cit_flag] = "?"
                }
                issn_isbn[cit_flag] = substr ( ($0), 36, 4)
                if (issn_isbn[cit_flag] == "    " && \
                  substr(jour_1[cit_flag],1,9) != "TO BE PUB") {
                  if (jrnl_pub_pub_1[cit_flag] != "?") {
                ...
                if (issn_isbn[cit_flag] != "ISBN") {
                isbn[cit_flag]      = "?"
                issn[cit_flag]      = substr ( ($0), 41, 25)
                } else {
                issn[cit_flag]      = "?"
                isbn[cit_flag]      = substr ( ($0), 41, 25)
                }
                csd[cit_flag]       = substr ( ($0), 67, 4)
                if (csd[cit_flag] == "    ") csd[cit_flag] = "?"
                                     }
                           }
}
#====================================================================
...
{
        if (first_field == "KEYWRD" || first_field == "KEYWDS") {
        xlat_flag=xlat_save

        keywrd_list             =       substr ( ($0), 11, 60)
#
        if ( keywrd_flag == 0 ) {
        keywrd_tail = ""
        ...
        dosplit(num_keys,keywrd_list, keyslist, comma )
        for (ii=1; ii <= num_keys; ++ii){
          dosplit(num_k_split,keyslist[ii], keys_split, " ")
          keyslist[ii] = ""
          if (num_k_split > 0) {
          ...
          key_tail = ""
          if (num_keys > 0) key_tail = keyslist[num_keys]
          }
        }
}
#===========================================================================
...
#
{
        ...
        lk_atom_beg[++link_flag]              = substr( ($0),13, 4)
        lk_alt_loc_beg[link_flag]             = substr( ($0),17, 1)
        lk_res_name_beg[link_flag]            = substr( ($0),18, 3)
        lk_chain_id_beg[link_flag]            = substr( ($0),22, 1)
        lk_res_seq_num_beg[link_flag]         = substr( ($0),23, 5)
        lk_atom_end[link_flag]                = substr( ($0),43, 4)
        lk_alt_loc_end[link_flag]             = substr( ($0),47, 1)
        lk_res_name_end[link_flag]            = substr( ($0),48, 3)
        lk_chain_id_end[link_flag]            = substr( ($0),52, 1)
        lk_res_seq_num_end[link_flag]         = substr( ($0),53, 5)
        lk_symm_1[link_flag]                  = substr( ($0),60, 6)
        lk_symm_2[link_flag]                  = substr( ($0),67, 6)


        dosqueez(temp_name,lk_atom_beg[link_flag])
        lk_atom_beg[link_flag] = temp_name
        dosqueez(temp_name,lk_atom_end[link_flag])
        lk_atom_end[link_flag] = temp_name
        if (lk_alt_loc_beg[link_flag] == " ") \
          lk_alt_loc_beg[link_flag] = "."
        if (lk_alt_loc_end[link_flag] == " ") \
          lk_alt_loc_end[link_flag] = "."
        if (lk_res_name_beg[link_flag] == "   ") \
          lk_res_name_beg[link_flag] = " . "
        if (lk_res_name_end[link_flag] == "   ") \
          lk_res_name_end[link_flag] = " . "
        if (lk_chain_id_beg[link_flag] == " ") \
          lk_chain_id_beg[link_flag] = bcid
        if (lk_chain_id_end[link_flag] == " ") \
          lk_chain_id_end[link_flag] = bcid
        if (lk_symm_1[link_flag] == "      ") {
          lk_symm_1[link_flag] = "   .   "
        } else {
          lk_symm_1[link_flag] = \
            (substr(lk_symm_1[link_flag],1,3) "_" \
            substr(lk_symm_1[link_flag],4,3))
        }
        if (lk_symm_2[link_flag] == "      ") {
          lk_symm_2[link_flag] = "   .   "
        } else {
          lk_symm_2[link_flag] = \
            (substr(lk_symm_2[link_flag],1,3) "_" \
            substr(lk_symm_2[link_flag],4,3))
        }
...
{
        if (first_field == "MASTER") {
        xlat_flag=xlat_save

          # parse totals

                total_remark    = substr( ($0), 11, 5)
                total_ftnote    = substr( ($0), 16, 5)
                total_het       = substr( ($0), 21, 5)
                total_helix     = substr( ($0), 26, 5)
                total_sheet     = substr( ($0), 31, 5)
                total_turn      = substr( ($0), 36, 5)
                total_site      = substr( ($0), 41, 5)
                total_o_s_m     = substr( ($0), 46, 5)
                total_a_h       = substr( ($0), 51, 5)
                total_ter       = substr( ($0), 56, 5)
                total_conect    = substr( ($0), 61, 5)
                total_seqres    = substr( ($0), 66, 5)
...
{
      if (first_field == "MODRES") {
        xlat_flag=xlat_save
        modres_resName[++modres_flag] = substr( ($0), 13,  3)
        modres_chainID[modres_flag]   = substr( ($0), 17,  1)
        if (modres_chainID[modres_flag] == " ") \
            modres_chainID[modres_flag] = bcid
        modres_seq[modres_flag]       = substr( ($0), 19,  5)
        modres_dbRes[modres_flag]     = substr( ($0), 25,  3)
        modres_conflict[modres_flag]  = substr( ($0), 30, 41)
        if (modres_dbSeq[modres_flag] == "      ") {
          modres_dbSeq[modres_flag] = "."
        }
        modres_conflict[modres_flag] = \
        ("Chain " modres_chainID[modres_flag] ": " \
        modres_conflict[modres_flag] )
        dosplit(numx,modres_conflict[modres_flag],dblist," ")
        modres_conflict[modres_flag] = "."
        if (numx > 0) {
          ...
          if (numx >1) modres_conflict[modres_flag] = \
            ("'" modres_conflict[modres_flag] "'")
        }
      }
}
...
#
{
        if (first_field == "MTRIX1" || first_field == "MTRIX2" \
          || first_field == "MTRIX3") {
        xlat_flag=xlat_save

        mtrix_col1 = substr ( ($0), 11, 10)
        mtrix_col2 = substr ( ($0), 21, 10)
        mtrix_col3 = substr ( ($0), 31, 10)
        mtrix_col4 = substr ( ($0), 46, 10)
       # print loop headers
 ...
 if (mtrix_flag == "0") {
   ...
   mat_save[++mtrix_flag] = "_struct_ncs_oper.vector[3] \n"
        ...
        mtrix_id                = substr( ($0), 8, 3)
        mtrix_given		= substr( ($0),60, 1)
        x_given			= "generate"
        if (mtrix_given != " ") x_given = "given"

        if (first_field == "MTRIX1") {
        mat_save[++mtrix_flag]=sprintf ("%3s    %s\n",  mtrix_id, x_given)
                            }
        mat_save[++mtrix_flag] = (mtrix_col1 " " \
                                  mtrix_col2 " " \
                                  mtrix_col3 " " \
                                  mtrix_col4 "\n")
...
{
  if (first_field == "ORIGX1" || first_field == "ORIGX2" \
    || first_field == "ORIGX3") {
    xlat_flag=xlat_save

        origx_col1 = substr ( ($0), 11, 10)
        origx_col2 = substr ( ($0), 21, 10)
        origx_col3 = substr ( ($0), 31, 10)
        origx_col4 = substr ( ($0), 46, 10)
    ...
    om_save[++origx_flag] = ("  "  head_PDB_code "\n\n")
        ...
        origx_id                = substr( ($0), 8, 3)
        om_save[++origx_flag] = (origx_col1 " " \
                                 origx_col2 " " \
                                 origx_col3 " " \
                                 origx_col4 "\n" )
  }
...
#
{  if (flush_refs == 1 ){
     remark_number           = substr ( ($0), 8,  3)
     if ((first_field == "REMARK" &&  \
       remark_number == "  2" && \
       $3 == "RESOLUTION.") || \
       ((first_field != "REMARK") && (first_field !="JRNL")) )
     {
...
#       _citation.details          == ?
        	...
        	if (country[i] == "  " || country[i] == "")    country[i] = "?"
        	if (jour_1[i] == "                            ") \
                                           jour_1[i] = "?"

        	if (volu[i] == "    " || volu[i] == "")     volu[i] = "?"
        	if (year[i] == "    " || year[i] == "")     year[i] = "?"
        	if (page[i] == "     " || page[i] == "")    page[i] = "?"
                if (csd[i] == "    " || csd[i] == "")       csd[i] = "?"

             if (cit_title_1[i]) printf ("; %s\n", cit_title_1[i])
             else                printf (" ? ")
             if (cit_title_2[i]) printf ("  %s\n", cit_title_2[i])
             if (cit_title_1[i]) printf (";\n")

             printf (" %2s ? %3s ? %5s %4s ? ? %4s\n" \
                       ,country[i],volu[i], page[i], year[i], csd[i])
        ...
        if (jrnl_pub_pub_1[i]) printf (";  %s \n", jrnl_pub_pub_1[i])
        if (jrnl_pub_pub_2[i]) printf ("   %s \n", jrnl_pub_pub_2[i])
        if (jrnl_pub_pub_1[i]) printf (";\n")
...
# for journals
        ...
        if (!jour_2[i]) {
   printf (";\n %2s '%28s' %4s  ?   %5s   %4s \n'%-15s' '%15s' %4s ? ? ? ?\n",\
         country[i], jour_1[i], volu[i], page[i], year[i], astm[i], \
         issn[i], csd[i])
        }
        if(jour_2[i]) {
   printf ((";\n %2s \n; %-28s\n  %s\n;\n"\
           " %4s  ?   %5s   %4s \n'%-15s' '%15s' %4s ? ? ? ?\n"),\
         country[i], jour_1[i], jour_2[i], volu[i], page[i], year[i], astm[i], \
         issn[i], csd[i])
        }
                           }
                                       }
# Loop Editor List
        ...
        if (cit_edit_1[i]) {
                dosplit(num_edit,cit_edit_1[i], editors, comma )
                for (ii=1; ii <= num_edit; ++ii){
                  dosplit(num_e_split,editors[ii], e_split, " ")
                  editors[ii] = ""
                  if (num_e_split > 0) {
                    editors[ii] = e_split[1]
                    for (j=2; j <= num_e_split; ++j) {
                      editors[ii] = (editors[ii] " " e_split[j])
                    }
                  }
                  if (auth_convtext == "yes" || \
                    (auth_convtext == "conditional" \
                    && convtext == "yes")) {
                      donameset(editors[ii],editors[ii])
                  }
                }
        	    if (cit_edit_2[i]) (num_edit = num_edit-1)
                for (j=1; j<= num_edit; ++j ) {
                  if ( (primary) && i == "1" )
                    printf (" primary   '%s' \n", editors[j])
                  else
                    printf (" %3s   '%s' \n", i-cit_decr, editors[j])
                }
        }

        if (cit_edit_2[i]) {
                dosplit(num_edit,cit_edit_2[i], editors, comma )
                for (ii=1; ii <= num_edit; ++ii){
                  dosplit(num_e_split,editors[ii], e_split, " ")
                  editors[ii] = ""
                  if (num_e_split > 0) {
...
# Loop Author List
        ...
        if (cit_auth_1[i]) {
        	dosplit(num_auth,cit_auth_1[i], authors, comma )
                for (ii=1; ii <= num_auth; ++ii){
                  dosplit(num_a_split,authors[ii], a_split, " ")
                  authors[ii] = ""
                  if (num_a_split > 0) {
                    authors[ii] = a_split[1]
                    for (j=2; j <= num_a_split; ++j) {
                      authors[ii] = (authors[ii] " " a_split[j])
                    }
                  }
                  if (auth_convtext == "yes" || \
                    (auth_convtext == "conditional" \
                    && convtext == "yes")) {
                      donameset(authors[ii],authors[ii])
                  }
                }
        	if (cit_auth_2[i]) (num_auth = num_auth-1)
        ...
        if (cit_auth_2[i]) {
                dosplit(num_auth,cit_auth_2[i], authors, comma )
                for (ii=1; ii <= num_auth; ++ii){
                  dosplit(num_a_split,authors[ii], a_split, " ")
                  authors[ii] = ""
                  if (num_a_split > 0) {
...
}
{
        ...
        remark_number           = substr ( ($0), 8,  3)
        remark_cont             = substr ( ($0),17,  2)
        jrnl_rec_type           = substr ( ($0),13,  4)
        jrnl_refNum             = substr ( ($0),22, 49)+0
        remark_text             = substr ( ($0),12, 60)
        remark_cit_text         = substr ( ($0),20, 51)
        if (convtext == "yes" && jrnl_rec_type == "TITL") {
          dotypeset(remark_cit_text,remark_cit_text)}
        remark_test             = substr ( ($0),12,  3)
...
#
  if (remark_number == "  4"){
    if (substr($0, 17, 23) == \
      "COMPLIES WITH FORMAT V.") \
      compliance_level = substr($0, 41,3)
  }
# type 1 remarks - additional references
# data items identical to JRNL

        if (remark_number == "  1" && remark_test == "REF") {
          ++cit_flag
          flush_refs = 1
...
# Assign TITL records
        ...
        if (jrnl_rec_type == "TITL" && remark_cont == "  ") {
           cit_title_1[cit_flag] = remark_cit_text
           cit_title_2[cit_flag] = ""
        }
        if (jrnl_rec_type == "TITL" && remark_cont != "  ") {
           if (  cit_title_2[cit_flag] == "" ) \
...
# Assign AUTH records
        ...
        if (jrnl_rec_type == "AUTH" && remark_cont == "  ") {
           cit_auth_1[cit_flag] = remark_cit_text
           cit_auth_2[cit_flag] = ""
        }
        if (jrnl_rec_type == "AUTH" && remark_cont != "  ")
           cit_auth_2[cit_flag] = \
             (cit_auth_2[cit_flag] remark_cit_text )

# Assign EDIT records

        if (jrnl_rec_type == "EDIT" && remark_cont == "  ") {
            cit_edit_1[cit_flag] = remark_cit_text
            cit_edit_2[cit_flag] = ""
        }
        if (jrnl_rec_type == "EDIT" && remark_cont != "  ")
            cit_edit_2[cit_flag] = \
              ( cit_edit_2[cit_flag]  remark_cit_text )

# Assign REF records

        if (jrnl_rec_type == "REF " && remark_cont == "  ") {
                jour_1[cit_flag]      = substr ( ($0), 20, 28)
                jour_2[cit_flag]      = ""
                volu[cit_flag]        = substr ( ($0), 52, 4)
                page[cit_flag]        = substr ( ($0), 57, 5)
        ...
        if (jrnl_rec_type == "REF " && remark_cont != "  ") {
            if ( jour_2[cit_flag] == "" ) \
              jour_2[cit_flag] = substr ( ($0), 20, 28)
            else \
        	jour_2[cit_flag]      = \
        	  ( jour_2[cit_flag] "\n  " substr ( ($0), 20, 28))
                                                            }
# Assign PUBL records
        ...
        if (jrnl_rec_type == "PUBL"  && remark_cont == "  ") {
             jrnl_pub_pub_1[cit_flag] = substr ( ($0), 20, 51)
             jrnl_pub_pub_2[cit_flag] = ""
        }
        if (jrnl_rec_type == "PUBL"  && remark_cont != "  ") {
...
#
        ...
        if (remark_number == "  2" && $3 == "RESOLUTION.") {

         resolution = substr ( ($0), 23, 45)

      dosplit(num_split,resolution, res_split, " ")
      if ( res_split[1] != "NOT" ) {
      res_flag = 1
      printf ("\n_reflns.entry_id             %s \n", head_PDB_code )
...
#   type 3-x remarks
        ...
        if (remark_number != "  1" && remark_number != "  2") {

        	if (remark_header_flag == "0")  {
        	   printf ("\nloop_\n")
...
#
{
        if (first_field == "REVDAT") {
        xlat_flag=xlat_save

                rev_mod_number[revdat_flag]	= substr( ($0),  8, 3)
                rev_date[revdat_flag]		= substr( ($0), 14, 9)
                if( rev_date[revdat_flag] == "         "){
                  rev_date[revdat_flag] = rev_date[revdat_flag-1]
                }
        	rev_cont_flag[revdat_flag]	= substr( ($0), 11, 2)
                rev_name[revdat_flag]		= substr( ($0), 24, 5)
                rev_type[revdat_flag]		= substr( ($0), 32, 1)
                rev_rec_corr[revdat_flag]	= substr( ($0), 40,31)
...
#
        if (revdat_flag == 1) aud_rev_id = rev_name[revdat_flag]
#
#       if first REVDAT of a block, save pointer for audit.update_record
#
        if (rev_cont_flag[revdat_flag] == "  " ) {
        	audit_flag++
...
#   change date format
        ...
        dosplit(cal_date,rev_date[revdat_flag], dmy, "-" )	
...
#
{
  if (first_field == "SPRSDE"  || first_field == "OBSLTE") {
        xlat_flag=xlat_save
    s_o_type[++s_o_flag] = first_field
    s_o_cont[s_o_flag] = substr( ($0), 9, 2)
    s_o_date[s_o_flag] = substr( ($0),12, 9)
    s_o_name[s_o_flag] = substr( ($0),22, 4)
    s_o_list[s_o_flag] = substr( ($0),32,39)
                           }
...
#
{
        if (first_field == "SCALE1" || first_field == "SCALE2" \
            || first_field == "SCALE3") {
        xlat_flag=xlat_save

        scale_col1	= substr( ($0), 11,10)
        scale_col2      = substr( ($0), 21,10)
        scale_col3      = substr( ($0), 31,10)
        scale_col4      = substr( ($0), 46,10)
        if (first_field == "SCALE1") {
        sc_save[++scale_flag] = "\n"
        sc_save[++scale_flag] = "loop_\n"
        sc_save[++scale_flag] = "_atom_sites.entry_id\n"
        sc_save[++scale_flag] = "_atom_sites.cartn_transform_axes\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[1][1]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[1][2]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[1][3]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_vector[1]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[2][1]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[2][2]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[2][3]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_vector[2]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[3][1]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[3][2]\n"
        sc_save[++scale_flag] = "_atom_sites.fract_transf_matrix[3][3]\n"
...
{
      if (first_field == "SEQADV") {
        xlat_flag=xlat_save
        seqadv_resName[++seqadv_flag] = substr( ($0), 13,  3)
        if (seqadv_resName[seqadv_flag] == "   ") \
            seqadv_resName[seqadv_flag] = " . "
        seqadv_chainID[seqadv_flag]   = substr( ($0), 17,  1)
        if (seqadv_chainID[seqadv_flag] == " ") \
            seqadv_chainID[seqadv_flag] = bcid
        seqadv_seq[seqadv_flag]       = substr( ($0), 19,  5)
        if (seqadv_seq[seqadv_flag] == "     ") \
            seqadv_seq[seqadv_flag] = "  .  "
        seqadv_database[seqadv_flag]  = substr( ($0), 25,  4)
        seqadv_dbAccession[seqadv_flag]\
                                      = substr( ($0), 30,  9)
        seqadv_dbRes[seqadv_flag]     = substr( ($0), 40,  3)
        seqadv_dbSeq[seqadv_flag]     = substr( ($0), 44,  5)
        seqadv_conflict[seqadv_flag]  = substr( ($0), 50, 21)
        dosplit(numx,seqadv_database[seqadv_flag],dblist," ")
        ...
        dosplit(numx,seqadv_dbAccession[seqadv_flag],dblist," ")
        seqadv_dbAccession[seqadv_flag] = "."
        if (numx > 0) {
        ...
        if (seqadv_dbRes[seqadv_flag] == "   ") {
          seqadv_dbRes[seqadv_flag] = " . "
        }
        if (seqadv_dbSeq[seqadv_flag] == "     ") {
          seqadv_dbSeq[seqadv_flag] = "  .  "
        }
        seqadv_conflict[seqadv_flag] = \
        ("Chain " seqadv_chainID[seqadv_flag] ": " \
        seqadv_conflict[seqadv_flag] )
        dosplit(numx,seqadv_conflict[seqadv_flag],dblist," ")
        seqadv_conflict[seqadv_flag] = "."
        if (numx > 0) {
          ...
          if (numx >1) seqadv_conflict[seqadv_flag] = \
            ("'" seqadv_conflict[seqadv_flag] "'")
        }
      }
}
...
#
{
        if (first_field == "SEQRES") {
        xlat_flag=xlat_save

                seq_record_number[seqres_flag] \
                                           = substr( ($0), 9, 2)
                seq_res_count[seqres_flag] = substr( ($0), 14,4)
                seq_chain_id[seqres_flag]  = substr( ($0), 12,1)
                seq_text[seqres_flag]      = substr( ($0), 20, 53)

                if (seq_chain_id[seqres_flag] == " ") \
...
{
  if (first_field == "SHEET") {
        xlat_flag=xlat_save


   #    Parse field

   sheet_strand_no[++sheet_flag]       = substr( ($0), 8, 3)+0
   sheet_id[sheet_flag]                = substr( ($0),12, 3)
   sheet_no_strands[sheet_flag]        = substr( ($0),15, 2)+0
   sheet_res_name_beg[sheet_flag]      = substr( ($0),18, 3)
   sheet_chain_id_beg[sheet_flag]      = substr( ($0),22, 1)
   sheet_res_seq_beg[sheet_flag]       = substr( ($0),23, 5)
   sheet_res_name_end[sheet_flag]      = substr( ($0),29, 3)
   sheet_chain_id_end[sheet_flag]      = substr( ($0),33, 1)
   sheet_res_seq_end[sheet_flag]       = substr( ($0),34, 5)
   sheet_sense[sheet_flag]             = substr( ($0),39, 2)
   dosqueez(sheet_atom_name_reg_1[sheet_flag],\036substr( ($0),42, 4)\037)
   sheet_res_name_reg_1[sheet_flag]    = substr( ($0),46, 3)
   sheet_chain_id_reg_1[sheet_flag]    = substr( ($0),50, 1)
   sheet_res_seq_reg_1[sheet_flag]     = substr( ($0),51, 5)
   dosqueez(sheet_atom_name_reg_2[sheet_flag],\036substr( ($0),57, 4)\037)
   sheet_res_name_reg_2[sheet_flag]    = substr( ($0),61, 3)
   sheet_chain_id_reg_2[sheet_flag]    = substr( ($0),65, 1)
   sheet_res_seq_reg_2[sheet_flag]     = substr( ($0),66, 5)

    if (sheet_chain_id_beg[sheet_flag] == " ") \
      sheet_chain_id_beg[sheet_flag] = bcid
    if (sheet_chain_id_end[sheet_flag] == " ") \
      sheet_chain_id_end[sheet_flag] = bcid
    if (sheet_sense[sheet_flag] == " 1") sheet_sense[sheet_flag] = "parallel"
    if (sheet_sense[sheet_flag] == "-1") \
      sheet_sense[sheet_flag] = "anti-parallel"
    if (sheet_res_name_reg_1[sheet_flag] == "   " || \
        sheet_res_name_reg_1[sheet_flag] == "") \
        sheet_res_name_reg_1[sheet_flag] = " . "
    if (sheet_chain_id_reg_1[sheet_flag] == " ") \
        sheet_chain_id_reg_1[sheet_flag] = bcid
    if (sheet_res_seq_reg_1[sheet_flag] == "     " || \
        sheet_res_seq_reg_1[sheet_flag] == "") \
        sheet_res_seq_reg_1[sheet_flag] = "  .  "
    if (sheet_res_name_reg_2[sheet_flag] == "   " || \
        sheet_res_name_reg_2[sheet_flag] == "") \
        sheet_res_name_reg_2[sheet_flag] = " . "
    if (sheet_chain_id_reg_2[sheet_flag] == " ") \
        sheet_chain_id_reg_2[sheet_flag] = bcid
    if (sheet_res_seq_reg_2[sheet_flag] == "     " || \
...

{
        ...
        sa_atom_serial_number[sigatm_flag] = substr( ($0),  7, 5)
        sigatm_point[(sa_atom_serial_number[sigatm_flag] "|" model_flag)] = \
                                        sigatm_flag
        sig_x[sigatm_flag]             = substr( ($0), 31, 8)
        sig_y[sigatm_flag]             = substr( ($0), 39, 8)
        sig_z[sigatm_flag]             = substr( ($0), 47, 8)
        sig_occ[sigatm_flag]           = substr( ($0), 55, 6)
        sig_temp[sigatm_flag]          = substr( ($0), 61, 6)
        if (sig_x[sigatm_flag] == "        " ) {
          sig_x[sig_flag] = "    .   " }
        if (sig_y[sigatm_flag] == "        " ) {
          sig_y[sigatm_flag] = "    .   " }
        if (sig_z[sigatm_flag] == "        " ) {
          sig_z[sigatm_flag] = "    .   " }
        if (sig_occ[sigatm_flag] == "      " ) {
          sig_occ[sigatm_flag] = "   .  " }
        if (sig_temp[sigatm_flag] == "      " ) {
...

{
        ...
        su_atom_serial_number[siguij_flag] = substr( ($0),  7, 5)
        siguij_point[(su_atom_serial_number[siguij_flag] "|" model_flag)] = \
                                           siguij_flag
        sig_U11[siguij_flag]             = substr( ($0), 29, 7)
        sig_U22[siguij_flag]             = substr( ($0), 36, 7)
        sig_U33[siguij_flag]             = substr( ($0), 43, 7)
        sig_U12[siguij_flag]             = substr( ($0), 50, 7)
        sig_U13[siguij_flag]             = substr( ($0), 57, 7)
        sig_U23[siguij_flag]             = substr( ($0), 64, 7)
...

{
        ...
        site_flag_1 = 1
        site_seq_no[site_flag]		   = substr( ($0),  8,3)
        site_id[site_flag]                 = substr( ($0), 12,3)
        site_no_res[site_flag]             = substr( ($0), 16,2)
        site_res_name_1[site_flag]         = substr( ($0), 19,3)
        site_res_name_2[site_flag]         = substr( ($0), 30,3)
        site_res_name_3[site_flag]	   = substr( ($0), 41,3)
        site_res_name_4[site_flag]	   = substr( ($0), 52,3)
        site_chain_id_1[site_flag]         = substr( ($0), 23,1)
        site_chain_id_2[site_flag]         = substr( ($0), 34,1)
        site_chain_id_3[site_flag]         = substr( ($0), 45,1)
        site_chain_id_4[site_flag]         = substr( ($0), 56,1)
        site_res_seq_1[site_flag]          = substr( ($0), 24,5)
        site_res_seq_2[site_flag]          = substr( ($0), 35,5)
        site_res_seq_3[site_flag]          = substr( ($0), 46,5)
        site_res_seq_4[site_flag]	   = substr( ($0), 57,5)

   if (site_chain_id_1[site_flag] == " ") site_chain_id_1[site_flag] = bcid
   if (site_chain_id_2[site_flag] == " ") site_chain_id_2[site_flag] = bcid
   if (site_chain_id_3[site_flag] == " ") site_chain_id_3[site_flag] = bcid
   if (site_chain_id_4[site_flag] == " ") site_chain_id_4[site_flag] = bcid
...
#
{
        if (first_field == "SLTBRG") {
        xlat_flag=xlat_save

        sb_atom_beg[++sltbrg_flag]              = substr( ($0),13, 4)
        sb_alt_loc_beg[sltbrg_flag]             = substr( ($0),17, 1)
        sb_res_name_beg[sltbrg_flag]            = substr( ($0),18, 3)
        sb_chain_id_beg[sltbrg_flag]            = substr( ($0),22, 1)
        sb_res_seq_num_beg[sltbrg_flag]         = substr( ($0),23, 5)
        sb_atom_end[sltbrg_flag]                = substr( ($0),43, 4)
        sb_alt_loc_end[sltbrg_flag]             = substr( ($0),47, 1)
        sb_res_name_end[sltbrg_flag]            = substr( ($0),48, 3)
        sb_chain_id_end[sltbrg_flag]            = substr( ($0),52, 1)
        sb_res_seq_num_end[sltbrg_flag]         = substr( ($0),53, 5)
        sb_symm_1[sltbrg_flag]	            = substr( ($0),60, 6)
        sb_symm_2[sltbrg_flag]	            = substr( ($0),67, 6)


        dosqueez(temp_name,sb_atom_beg[sltbrg_flag])
        sb_atom_beg[sltbrg_flag] = temp_name
        dosqueez(temp_name,sb_atom_end[sltbrg_flag])
        sb_atom_end[sltbrg_flag] = temp_name
        if (sb_alt_loc_beg[sltbrg_flag] == " ") \
          sb_alt_loc_beg[sltbrg_flag] = "."
        if (sb_alt_loc_end[sltbrg_flag] == " ") \
          sb_alt_loc_end[sltbrg_flag] = "."
        if (sb_res_name_beg[sltbrg_flag] == "   ") \
          sb_res_name_beg[sltbrg_flag] = " . "
        if (sb_res_name_end[sltbrg_flag] == "   ") \
          sb_res_name_end[sltbrg_flag] = " . "
        if (sb_chain_id_beg[sltbrg_flag] == " ") \
          sb_chain_id_beg[sltbrg_flag] = bcid
        if (sb_chain_id_end[sltbrg_flag] == " ") \
          sb_chain_id_end[sltbrg_flag] = bcid
        if (sb_symm_1[sltbrg_flag] == "      ") {
          sb_symm_1[sltbrg_flag] = "   .   "
        } else {
          sb_symm_1[sltbrg_flag] = \
            (substr(sb_symm_1[sltbrg_flag],1,3) "_" \
            substr(sb_symm_1[sltbrg_flag],4,3))
        }
        if (sb_symm_2[sltbrg_flag] == "      ") {
          sb_symm_2[sltbrg_flag] = "   .   "
        } else {
          sb_symm_2[sltbrg_flag] = \
            (substr(sb_symm_2[sltbrg_flag],1,3) "_" \
            substr(sb_symm_2[sltbrg_flag],4,3))
        }
...
{
        if (first_field == "SOURCE") {
        xlat_flag=xlat_save

        	source_cont           = substr( ($0), 9, 2)
                source_text           = substr( ($0), 11, 60)
                if (convtext == "yes") \
                  dotypeset(source_text,source_text)
...
#
{
        if (first_field == "SSBOND") {
        xlat_flag=xlat_save

        
        ssbond_num[ssbond_flag]		        = substr( ($0), 8, 3)
        ssbond_res_name_beg[ssbond_flag]	= substr( ($0),12, 3)
        ssbond_chain_id_beg[ssbond_flag]	= substr( ($0),16, 1)
        ssbond_res_seq_num_beg[ssbond_flag]	= substr( ($0),18, 5)
        ssbond_res_name_end[ssbond_flag]	= substr( ($0),26, 3)	
        ssbond_chain_id_end[ssbond_flag]	= substr( ($0),30, 1)
        ssbond_res_seq_num_end[ssbond_flag]	= substr( ($0),32, 5)
        ssbond_symm_1[ssbond_flag]		= substr( ($0),60,6)
        ssbond_symm_2[ssbond_flag]		= substr( ($0),67,6)

        if (ssbond_chain_id_beg[ssbond_flag] == " ") \
            ssbond_chain_id_beg[ssbond_flag] = bcid
        if (ssbond_chain_id_end[ssbond_flag] == " ") \
            ssbond_chain_id_end[ssbond_flag] = bcid
        if (ssbond_symm_1[ssbond_flag] == "      ") {
          ssbond_symm_1[ssbond_flag] = "   .   "
        } else {
          ssbond_symm_1[ssbond_flag] = \
            (substr(ssbond_symm_1[ssbond_flag],1,3) "_" \
            substr(ssbond_symm_1[ssbond_flag],4,3))
        }
        if (ssbond_symm_2[ssbond_flag] == "      ") {
          ssbond_symm_2[ssbond_flag] = "   .   "
        } else {
          ssbond_symm_2[ssbond_flag] = \
            (substr(ssbond_symm_2[ssbond_flag],1,3) "_" \
            substr(ssbond_symm_2[ssbond_flag],4,3))
        }
...

{
        ...
        xlat_flag=xlat_save
          ...
          title_cont           = substr( ($0), 9, 2)
          title_text           = substr( ($0), 11, 60)
          bp = "     "
          if (title_cont != "  ") bp = "    "
...

{
        ...
        xlat_flag=xlat_save
         ...
         turn_num[ss_flag]                   = substr( ($0), 8, 3)
         turn_id[ss_flag]                    = substr( ($0),12, 3)
         turn_res_name_beg[ss_flag]          = substr( ($0),16, 3)
         turn_chain_id_beg[ss_flag]          = substr( ($0),20, 1)

         turn_res_seq_beg[ss_flag]           = substr( ($0),21, 5)
         turn_res_name_end[ss_flag]          = substr( ($0),27, 3)
         turn_chain_id_end[ss_flag]          = substr( ($0),31, 1)

         turn_res_seq_end[ss_flag]           = substr( ($0),32, 5)
         turn_comment[ss_flag]               = substr( ($0),41,30)


# strip blanks from id
        dosplit(num_x,turn_id[ss_flag],xxx," ")
        turn_id[ss_flag] = ""
        if (num_x == 1) turn_id[ss_flag] = xxx[1]
        if (num_x == 2) turn_id[ss_flag] = (xxx[1] "_" xxx[2])

     if (turn_chain_id_beg[ss_flag] == " ") turn_chain_id_beg[ss_flag] = bcid
     if (turn_chain_id_end[ss_flag] == " ") turn_chain_id_end[ss_flag] = bcid

# give real name to turn classes (should all be TURN_P, but might as
# well check)

        t_class_suffix = "_P"
        dosplit(num_x,(turn_res_name_end[ss_flag] " " \
                        turn_res_name_beg[ss_flag]), xxx, " ")
        for(i in na_list) {
          if (na_list[i] == xxx[1] || \
              na_list[i] == xxx[2] ) t_class_suffix = "_N"
        }
...
{
        if (first_field == "TVECT") {
        xlat_flag=xlat_save

        tvect_id   = substr ( ($0),  8,  3)
        tvect_ent1 = substr ( ($0), 11, 10)
        tvect_ent2 = substr ( ($0), 21, 10)
        tvect_ent3 = substr ( ($0), 31, 10)
        tvect_com  = substr ( ($0), 41, 30)
...
END {
  ...
  for (is=1; is < seqres_flag; ++is) {
    dosplit(num_seq,seq_text[is],seq_res," ")
    stutter = 0
    if ( seq_record_number[is] == " 0" ) {
    ...
    for (i=1; i <= num_seq; ++i) {
      ...
      found = "no"
      for (j in na_list) {
        if (na_list[j] == cur_res) {
          if (substr(entities_list[k],1,3) == "Pro") {
            entities_list[k] = ("Protein/Nucleic Acid chain: " entities[k])
          } else {
            entities_list[k] = ("Nucleic Acid chain: " entities[k])
          }
          found = "yes"
          break
        }
      ...
      if (found == "no") {
      for (j in aa_list) {
        if (aa_list[j] == cur_res) {
          if (index(entities_list[k],"Nuc") > 0) {
            entities_list[k] = ("Protein/Nucleic Acid chain: " entities[k])
          } else {
            entities_list[k] = ("Protein chain: " entities[k])
          }
          break
        }
      }
      ...
      if (res_code[cur_res] ==  "") {
        ++num_res_name
        if(num_res_name > length(charx)) num_res_name -= length(charx)
        res_code[cur_res] = substr(charx,num_res_name,1)
      }
      seq_chain[ent_poly_num[seq_chain_id[is]]] = \
        (seq_chain[ent_poly_num[seq_chain_id[is]]] res_code[cur_res])
    }
  }
  for (i=1; i <= num_poly_ents; ++i) {
    for (j=1; j < length(seq_chain[i]); ++j) {
      code_pair = substr(seq_chain[i],j,2)
      if (pair_code[code_pair] == "") {
        ++num_res_pair
        if(num_res_pair > length(numl)) num_res_pair -= length(numl)
        pair_code[code_pair] = substr(numl,num_res_pair,1)
      }
      seq_dig = substr(seq_sig[i],pair_code[code_pair]+1,1)
      seq_dig = seq_dig+1
      if (seq_dig > 9) seq_dig -= 10
      seq_sig[i] = \
        (substr(seq_sig[i],1,pair_code[code_pair]) seq_dig \
         substr(seq_sig[i],pair_code[code_pair]+2, \
  ...
  for (i=1; i < num_poly_ents; ++i) {
    for (j=i+1; j <= num_poly_ents; ++j) {
      doseq_comp(xcomp,seq_sig[i],seq_sig[j],seq_match)
      if (seq_sig[i] == seq_sig[j]) {
        if (seq_chain[i] == seq_chain[j]) {
          entity_seq_num[entities[j]] = entity_seq_num[entities[i]]
          el_point[i] += 3
          xtemp = ", "
          if (el_point[i] >= 70) {
            xtemp = ",\n  "
          ...
          entities_list[i] = \
            (entities_list[i] xtemp entities[j])
        }
      } else {
        if ( xcomp > 85 ) {
        printf("# **** WARNING **** approx %5d%% homology %3s to %3s \n",\
  ...
  while (next_non_poly != "") {
    if (entity_seq_num[next_non_poly] == "" || \
  ...
  if(x_time == "") {
  x_time = system \
    (("/bin/echo `date +%Y-%m-%d` ' Converted to mmCIF \036\036format\037\037 by pdb2cif.pl'" \
     " " "VERSION()"))
  } else {
  printf((substr(x_time,21,4) "-" mmm2mm[substr(x_time,5,3)] "-" \
    substr(substr(x_time,9,2)+100,2,2) \
    ("  Converted to mmCIF \036\036format\037\037 by pdb2cif.pl" " " "VERSION()\n")))},
  ifdef(\036NOFUNCS\037,
  printf("????-??-??  Converted to mmCIF \036\036\036format\037\037\037 by pdb2cif.oawk VERSION()\n")
  warning_list[++warning_flag] = \
    "#=# AUDIT: Unable to obtain current date\n"
  ,
  system \
    (("/bin/echo `date +%Y-%m-%d` ' Converted to mmCIF \036\036\036format\037\037\037 by pdb2cif'" \
      " " "VERSION()"))))
  ...
  if (xlat_flag > 0) {
    ...
    warning_list[++warning_flag] = sprintf( \
      "#=# AUDIT: Did not translate %6d PDB records\n", xlat_flag)
  }
...
# Print out _struct_biol_info
  ...
  if (seqres_flag+0 > 1)  {
    ...
    for (is=1; is < seqres_flag; ++is) {
      if (ent_poly_num[seq_chain_id[is]] == \
        entity_seq_num[seq_chain_id[is]]) {
      if (iscurr != seq_chain_id[is]) {
        seq_start_num[entity_seq_num[seq_chain_id[is]]] = seq_number
        if (iscount > 0) {
          printf ("\n")
          iscount = 0
        }
  ...
  while ( cur_model_count < model_count && i > model_asn_high[cur_model_count] ) {
    cur_model_count++
  ...
  "# *** WARNING *** At atom number %5s unidentified entity %3s found \n",\
   atom_number[i], entity_id[i]) 
     ...
     warning_list[++warning_flag] = sprintf(\
  "#=# ATOM_SITE: At atom number %5s unidentified entity %3s found \n",\
   atom_number[i], entity_id[i]) 
  }
  #
  ...
  atom_ent_seq_num[i]="."
  if (prior_res_name != \
    (entity_id[i] residue_seq_number[i]) || \
    atom_or_het[i] == "TER" ) {
    ...
    if(prior_ats > 0 && pprior_ats > 0 && ppoxt_found == 0 ) {
      res_dist_sq = (prior_x-pprior_x)*(prior_x-pprior_x) \
        + (prior_y-pprior_y)*(prior_y-pprior_y) \
        + (prior_z-pprior_z)*(prior_z-pprior_z)
      res_max_sq = (pprior_ats+prior_ats)*(pprior_ats+prior_ats)*2
      if (res_max_sq < 25) res_max_sq = 25
      pnum = pprior_ats+1
      if (prior_ats < pnum) pnum = prior_ats+1
      res_min_sq = 2.5
      at_bonded = "1"
    ...
    al_res_z = 0
    al_res_ats = 0
    dosplit(x_num,residue_name[i],xxx," ")
    ...
    if (ppprior_res_name != "" && pprior_block != "" && \
      pprior_block == ppprior_block) \
    al_bond[pprior_block] = \
      (al_bond[pprior_block] at_bonded)
    if(atom_or_het[i] != "TER") {
      al_seq[(entity_id[i] "|" cur_model)] = \
        (al_seq[(entity_id[i] "|" cur_model)] res_code[cur_res])
      al_back_point[(length(al_seq[(entity_id[i] "|" cur_model)]) \
        (entity_id[i] "|" cur_model))] = i
    }
  }
  if(atom_name[i] == " OXT") oxt_found = 1
  if(atom_or_het[i] == "TER" ) {
    oxt_found = 1
    prior_block = ""
    pprior_block = ""
    ...
    if ((substr(atom_type[i],1,2) != " H") && \
      (substr(atom_type[i],1,2) != " D") && \
      (substr(atom_type[i],1,2) != " T") ) {
      al_res_x += atom_x[i]
      al_res_y += atom_y[i]
      al_res_z += atom_z[i]
      al_res_ats++  
  ...
  if (at_res_ats > 0) {
    ...
    if (pprior_res_name != "" && prior_block != "" && \
      pprior_block == prior_block) \
      al_bond[prior_block] = \
      (al_bond[prior_block] at_bonded)
  }
  for (iandm in al_seq) {
    dosplit(inum,iandm,xxiandm,"|")
    i = xxiandm[1]
    mymodel = xxiandm[2]
    al_seq_c[i] = ""
  }
  for (iandm in al_seq) {
    ...
    if (al_seq_c[i] != "") {
      if (al_seq[iandm] != al_seq_c[i]) {
        printf( \
         "# *** WARNING *** sequence mismatch chain %3s, model %s\n",\
           i, mymodel)
        warning_list[++warning_flag] = sprintf( \
        "#=# ATOM_SITE: Sequence mismatch chain %3s, model %s\n",\
          int(xmat), i, mymodel)
      }
    }
    if(nn_res <= num_poly_ents && al_seq_c[i] == "") {
      known_links = al_bond[iandm]
      doseq_comp(xmat,al_seq[iandm],seq_chain[nn_res],seq_match)
      if ( xmat < 90) {
        printf("# *** WARNING *** only %5d%% homology to chain %3s\n",\
          int(xmat), i)
        warning_list[++warning_flag] = sprintf( \
        "#=# ATOM_SITE: Only %5d%%%% homology to chain %3s\n",\
          int(xmat), i)
      }
      ...
      for(j=1; j<=length(al_seq[iandm]); ++j) {
        if(seq_match[j] >  0) {
          ...
          atom_ent_seq_num[k] = seq_match[j]+seq_start_num[nn_res]-1
      ...
      for(j=1; j<=length(al_seq[iandm]); ++j) {
        if(seq_match[j] <= 0 ) {
          ...
          if (oepsn != "" && oepsn = -seq_match[j]+seq_start_num[nn_res]-1) {
      ...
      if (misalign > 0 ) {
        warning_list[++warning_flag] = sprintf( \
        "#=# ATOM_SITE: %d Unmatched residues in chain %3s\n",\
           misalign, i)        
      }
    }
  }
  ...
  if (prior_res_name != \
    ...
    al_res_name[(chain_id[i] residue_seq_number[i])] = \
      residue_name[i]
    if (atom_ent_seq_num[i] == ".") {
      oepsn = ent_pol_seq_num[prior_res_name]
      if (oepsn != "") atom_ent_seq_num[i] = oepsn
    }
  }
  if (dense_list != "yes" && atom_ent_seq_num[i] == ".") {
    oepsn = ent_pol_seq_num[prior_res_name]
    if (oepsn != "") atom_ent_seq_num[i] = oepsn
  }
  }
  ...
  for (idb=1; idb <= dbref_flag; ++ idb) {
    ...
    dbref_seq_dif[ \
      (cid "|" dbn "|" dbref_dbAccession[idb])] = "no"
    ...
    if (dbref_seqEnd[idb] != "     ") {
      ...
      if (dblsn == "") dblsn = length( seq_chain[esn] )
    }
    dbtfsn = ""
    dbtlsn = ""
    if (dbref_dbseqBeg[idb] != "      " || \
      dbref_dbseqEnd[idb] != "      " ) {
      dbtfsn = dbref_dbseqBeg[idb]
      dbtlsn = dbref_dbseqEnd[idb]
      if (dbref_database[idb] == "PDB") {
        dbtfsn = ent_pol_seq_num[( cid dbtfsn ) ]
        if (dbtfsn == "") dbtfsn = 1
        dbtlsn = ent_pol_seq_num[( cid dbtlsn ) ]
        if (dbtlsn == "") dbtlsn = length( seq_chain[esn] )
      } else {
        dbtfsn += 0
        dbtlsn += 0
      }
    }
    if (dbfsn == "" && dblsn == "" && dbtfsn == "" && dbtlsn =="") {
      dbref_align[idb] = "."
      --num_align
      if (modres_flag > 0 && idb >  xdbref_flag) {
        dbref_seq_dif[ \
        (cid "|" dbn "|" dbref_dbAccession[idb])] = "yes"
      } else {
        dbref_esn[idb] = "."
        cid = head_PDB_code
    ...
    while (reloop == "yes") {
      if (next_ref == "" ){
        ...
        xdbid = (dbref_dbAccession[next_ref]  dbref_dbIdCode[next_ref] )
        if ( cid < xcid || \
          (cid == xcid && dbn < xdbn) || \
          (cid == xcid && dbn == xdbn && \
           dbid < xdbid)) { 
  ...
  if ( model_flags == "yes" ) {
    ...
    for (imodel = 1; imodel <= model_count; imodel++ ) {
      ...
      jasn = jaflo
      while ( iasn <= iafhi) {
        if ( jasn > jafhi || atom_name[iasn] != atom_name[jasn] || \
          residue_name[iasn] != residue_name[jasn] || \
          atom_alt_location[iasn] != atom_alt_location[jasn] || \
          chain_id[iasn] != chain_id[jasn] || \
          residue_seq_number[iasn] != residue_seq_number[jasn] ) {
          imatch = "false"
          kerror++
        } else {
          if ( root_model_rep[iasn] == "" && imodel == 1 ) {
            root_model_rep[iasn] = iasn
          }
          if ( root_model_rep[jasn] == "" ) {
            root_model_rep[jasn] = root_model_rep[iasn]
          }
        }
      alres = ( residue_name[iasn] "|" \
        chain_id[iasn] "|" residue_seq_number[iasn] )
      ...
      if ( kpoint == "" ) {
        ...
        reslist[alres] = iasn
        next_atom[iasn] = ""
        if ( root_model_rep[iasn] == "" ) {
          root_model_rep[iasn] = iasn
        }
      } else {
        alatom = ( atom_name[iasn] "|" atom_alt_location[iasn] )
        while ( kpoint != "" ) {
          blatom = ( atom_name[kpoint] "|" atom_alt_location[kpoint] )
          if ( alatom != blatom ) {
            lpoint = kpoint
            ...
            if ( kpoint == "" ) {
              ...
              if (imodel-1 != 0) {
                warning_list[++warning_flag] = \
                ("#=# ATOM_SITE: Model " model[imodel] \
               " added new atom " iasn " " atom_number[iasn] "\n")
              }
              if ( root_model_rep[iasn] == "" ) {
                root_model_rep[iasn] = iasn
              }
            ...
            if ( imodel == 1 ) {
              warning_list[++warning_flag] = \
              ("#=# ATOM_SITE: Atom " iasn " duplicates atom " kpoint "\n")
            }
      ...
      if ( imatch != "true" ) {
        warning_list[++warning_flag] = \
          ("#=# ATOM_SITE: Model " model[imodel] \
           " fails to match " kerror " atoms to next model \n") 
      }
    }
  ...
  for (i=1; i <= num_poly_ents; ++i) {
    if (ent_poly_num[entities[i]] == \
        entity_seq_num[entities[i]]) {
    printf (" %5d     polymer\n; %s\n;\n",  \
      entity_seq_num[entities[i]],entities_list[i])
    }
  }
  for (i=num_poly_ents+1; i < num_ents; ++i) {
    if ( entities[i] != "HOH" && entities[i] != "DOD") {
  ...
  for (hnames in het_site_name) {
    ...
    if (hsyn != "") {
      dosplit(num_hsyn,hsyn,hsyns,";")
      for (i=1; i <= num_hsyn; ++i) {
        xhsyn = hsyns[i]
        dosplit(num_xhsyn,xhsyn,xhsyns," ")
        yhsyn = ""
        ll = 0
        for (ii = 1; ii <= num_xhsyn; ++ii) {
          if (length(xhsyns[ii])+ll > 77) {
            if (ll > 0) {
              ...
              yhsyn = (yhsyn substr(xhsyns[ii],1,77) "\n  ")
  ...
  if (num_dbref+0 > 0 ){
    ...
    for (idb=1; idb <= dbref_flag; ++idb) {
      dbn = dbref_database[idb]
      cid = dbref_chainID[idb]
      xnumdbref = dbref_id[(dbref_chainID[idb] "|" dbref_database[idb] \
                "|" dbref_dbAccession[idb])]
      ...
      dbtfsn = "   .   "
      dbtlsn = "   .   "
      if (dbref_fsn[idb] != "") dbfsn = dbref_fsn[idb]
      if (dbref_lsn[idb] != "") dblsn = dbref_lsn[idb]
      if (dbref_tfsn[idb] != "") dbtfsn = dbref_tfsn[idb]
      if (dbref_tlsn[idb] != "") dbtlsn = dbref_tlsn[idb]
      dbid = (dbref_dbAccession[idb] dbref_dbIdCode[idb])
      if (dbref_align[idb] == "partial" || \
        (dbref_align[idb] == "." && idb > xdbref_flag)) \
      printf(" %5d %5d %7s %7s %7s %7s . \n", idb,xnumdbref, \
              dbfsn, dblsn, dbtfsn, dbtlsn)
    }
    ...
    for (isq = 1; isq <= modres_flag; ++isq) {
      ...
      printf(("%5d %5s %3s %5s %3s\n" \
          "         %s\n"), \
      dbref_align_id[(cid "|.|.")], \
         seq_num, resName, ".", db_resName, comment)
    } 
    }
  }
  if (verbose == "yes") {
  ...
  if (formul_flag > 1 || atom_res_flag > 0 || res_res_flag > 0) {
    ...
    if (formul_het_cont_flag[i] == "  ") {
      ...
      if (het_details == "") het_details = " "
      if (hetnam_flag+0 > 1) {
      het_name = het_site_name[formul_het_site_symbol[i]]
      if (het_name == "") het_name = " "
      printf("%s %s%s\n;\n; %s\n;\n; %s\n;\n",
        formul_het_site_symbol[i], \
        mon_ns, het_formula[formul_het_site_symbol[i]],\
        het_name,het_details)
     } else {
      printf("%s %s%s\n;\n; %s\n;\n",
        formul_het_site_symbol[i], \
        mon_ns, het_formula[formul_het_site_symbol[i]],\
    ...
    if (res_count[x_res_code]+0 > 0) {
      mon_formul = "."
      if (res_formul[x_res_code] != ".") mon_formul = \
        ("'" res_formul[x_res_code] "'")
      mon_name = "."
      if (res_name[x_res_code] != ".") mon_name = \
        ("\"" res_name[x_res_code] "\"")
      mon_ns = "yes"
      mon_det = " "
      if (hetnam_flag+0 > 1) mon_det = "."
...
# PRINT TVECT INFO
  ...
  if (atom_flag_1 == "1" && atom_flag+0 > 1) {
    printf ("\n\n\n")
    printf ("####################\n")
    printf ("#                  #\n")
    printf ("# ATOM_SITE        #\n")
  ...
  xes = ""}
  xs = ""
  if (substr( atom_x[i], 1, 1) != " ") {
  xs = "\n                             "}
  ys = ""
  if (substr( atom_y[i], 1, 1) != " ") {
  ys = "\n                                     "}
  zs = ""
  if (substr( atom_z[i], 1, 1) != " ") {
  zs = "\n                                             "}
  os = ""
  if (substr( atom_occ[i], 1, 1) != " ") {
  os = "\n                                                     "}
  bs = ""
  if (substr( B_or_U[i], 1, 1) != " ") {
  bs = "\n                                                           "}
  aco = ""
  if(atom_or_het[i] == "TER" && print_ter == "comment") {
    aco = "#"
    ts = " "
    if (substr(atom_type[i], 3, 2) != "") ts = "\n#         "
    if (compliance_level >= 2.0) {
  ...
  if (model_flags == "yes") {
    ...
    if ( id_in_model+0 == 0 ) {
      id_in_model = atom_id_by_type[( atom_name[i] "|" residue_name[i] "|" \
        atom_alt_location[i] "|" chain_id[i] "|" \
        residue_seq_number[i] )]
      auth_id_in_model = atom_number[atom_point_id_in_model[id_in_model]]
    }
    if ( id_in_model+0 == 0 ) {
      id_in_model = "."
      auth_id_in_model = "."
    ...
    if ( verbose != "yes" ) {
      es = (" " cur_model " " id_in_model " " auth_id_in_model es)
    } else {
      es = (" " cur_model " " id_in_model " " auth_id_in_model  " ." es)
    }
    if (atom_unique_ids == "no") {
      anum = (cur_model_count-1)*max_atom_number + auth_id_in_model
    }
  }
  ans = " "
  ...
  if (atom_or_het[i] != "TER" || print_ter != "no" ) {
  if (compliance_level >= 2.0) {
  printf \
("%s%s%-4s %s%s%s%4s %4s %1s %5s %1s%s%8s%s%8s%s%8s%s%6s%s%6s %s %3d%s%6d\n",\
    (aco atom_ent_seq_num[i]),es, \
    substr(atom_or_het[i],1,4),xes,atom_type[i],ts,atom_name[i], \
    residue_name[i], chain_id[i], \
    residue_seq_number[i],\
    atom_alt_location[i], xs, atom_x[i], ys, atom_y[i], zs, \
    atom_z[i], os, atom_occ[i], bs, B_or_U[i], \
    footnote_number[i], entity_seq_num[entity_id[i]], \
    ans, anum )
  } else {
  printf \
  ("%s%s%-4s %s%s%4s %4s %1s %5s %1s%s%8s%s%8s%s%8s%s%6s%s%6s %s %3d%s%6d\n", \
    (aco atom_ent_seq_num[i]),es, \
    substr(atom_or_het[i],1,4),atom_type[i],ts,atom_name[i], \
    residue_name[i], chain_id[i], \
    residue_seq_number[i],\
    atom_alt_location[i], xs, atom_x[i], ys, atom_y[i], zs, \
    atom_z[i], os, atom_occ[i], bs, B_or_U[i], \
  ...
  if (B_or_U[i] != "   .  ") {
    if (B_or_U[i]+0 > 0) { ++b_warn
    }
  }
  if (atom_occ[i] != "   .  ") {
    if (atom_occ[i]+0 < 0 || atom_occ[i]+0 > 1) { ++occ_warn
    }
    if (nmr_flag > 0) {
  ...
  if (sigatm_flag > 0 && \
    (atom_or_het[i] != "TER" || \
    (print_ter != "no" && print_ter != "comment"))) {
    x = sigatm_point[(atom_number[i] "|" cur_model)]
    ys = ""
    if (substr( sig_y[x], 1, 1) != " ") {
      ys = "\n                                     "}
    zs = ""
    if (substr( sig_z[x], 1, 1) != " ") {
      zs = "\n                                             "}
    os = ""
    if (substr( sig_occ[x], 1, 1) != " ") {
      os = "\n                                                     "}
    bs = ""
    if (substr( sig_temp[x], 1, 1) != " ") {
      bs = "\n                                                           "}
    if (x != "") {
      printf \
        ("                             %8s%s%8s%s%8s%s%6s%s%6s\n", \
        sig_x[x],ys,sig_y[x],zs,sig_z[x],os,sig_occ[x],bs,sig_temp[x])
        } else {
      printf \
      ("                                 .        .        .       .      .\n")
    }
  ...
  if (aniso_flag > 0 && \
    ...
    if (x != "") {
      printf ("                       %7g %7g %7g %7g %7g %7g \n", \
        atom_U11[x]/10000, atom_U12[x]/10000, atom_U13[x]/10000  , \
        atom_U22[x]/10000, atom_U23[x]/10000, atom_U33[x]/10000 )
    } else {
      printf ("    .       .       .       .       .       .\n")
    }
  } # end if (aniso_flag > 0)
  if (siguij_flag > 0 && \
    ...
    if (x != "") {
      printf ("                       %7g %7g %7g %7g %7g %7g \n", \
        sig_U11[x]/10000, sig_U12[x]/10000, sig_U13[x]/10000  , \
        sig_U22[x]/10000, sig_U23[x]/10000, sig_U33[x]/10000 )
    } else {
      printf ("    .       .       .       .       .       .\n")
    }
  } # end if (siguij_flag > 0)
...
#
  ...
  if (revdat_flag != "1" || s_o_flag != 0) {
    ...
    printf ("#                  #\n")
    printf ("####################\n\n\n")
    dosplit(null,head_dep_date, odmy, "-")
    printf ("\nloop_\n")
    ...
    for (j=1; j < revdat_flag; ++j) {
      if ( rev_cont_flag[j] == "  " ) {
        ...
        dosplit(my_s_o,xrev_s_o,x_s_o," ")
        if (my_s_o == 0 || x_s_o[1] == "" || x_s_o[1] == " ") {
          printf(" %4s-%2s-%2s ",\
          yyyy[odmy[3]+0],mmm2mm[odmy[2]],odmy[1])
          printf ("%3s  %4s-%2s-%2s  %1s . . .\n", \
            rev_mod_number[j], rev_date_year[j], rev_date_mon[j], \
            rev_date_day[j], rev_type[j])
        } else {
          for (k = 1; k <= my_s_o; ++k) {
            i = x_s_o[k]
            s_o_s = "."
            ...
            my_type = s_o_type[i]
            my_list = s_o_list[i]
            dosplit(my_count,my_list, my_revs, " ")
            for (l=1; l <= my_count; ++ l) {
              ...
              rev_mod_number[j], rev_date_year[j], rev_date_mon[j], \
              rev_date_day[j], rev_type[j], s_o_stat, s_o_o, s_o_s)
            }
    ...
    for (j=1; j < revdat_flag; ++j) {
      ...
      dosplit(rev_num_change,rev_rec_corr[j], rev_recs, " " )
  ...
  #
  {
    if (connect_flag+0 > 0 || ssbond_flag > 1) {
      ...
      printf (("_struct_conn." pdb2cif_prefix "ptnr1_atom_site_id\n"))
      printf ("_struct_conn.ptnr1_label_seq_id\n")
      printf (("_struct_conn." pdb2cif_prefix "ptnr2_atom_site_id\n"))
      ...
      for(icon = 1; icon <= connect_flag; ++ icon) {
        ...
        for (ipos = 12 ; ipos <= 57; ipos += 5) {
          connect_tar[++itarget] = substr( connect_save[icon], ipos, 5)
          c1 = 0+connect_source
          c2 = 0+connect_tar[itarget]
          if ( connect_tar[itarget]  != "     " ) {
            if ((chain_id[atom_point[c1]] residue_seq_number[atom_point[c1]] \
              atom_name[atom_point[c1]] atom_alt_location[atom_point[c1]] ) > \
              (chain_id[atom_point[c2]] residue_seq_number[atom_point[c2]] \
              atom_name[atom_point[c2]] atom_alt_location[atom_point[c2]] )) {
              ct = c1
              c1 = c2
              c2 = ct
            } # end if ((chain_id[atom_point[c1]] residue_seq_number[atom_point[c1]] ...
            conect_ptnr1_label_res_id  = residue_name[atom_point[c1]]
            conect_ptnr1_label_asym_id = chain_id[atom_point[c1]]
            conect_ptnr1_auth_seq_id   = residue_seq_number[atom_point[c1]]
            conect_ptnr1_label_atom_id = atom_name[atom_point[c1]]
            conect_ptnr1_label_alt_id  = atom_alt_location[atom_point[c1]]
            conect_ptnr2_label_res_id  = residue_name[atom_point[c2]]
            conect_ptnr2_label_asym_id = chain_id[atom_point[c2]]
            conect_ptnr2_auth_seq_id   = residue_seq_number[atom_point[c2]]
            conect_ptnr2_label_atom_id = atom_name[atom_point[c2]]
            conect_ptnr2_label_alt_id  = atom_alt_location[atom_point[c2]]
            connect_bond =\
              (conect_ptnr1_label_res_id \
              conect_ptnr1_label_asym_id conect_ptnr1_auth_seq_id \
              conect_ptnr1_label_atom_id conect_ptnr1_label_alt_id \
              " " conect_ptnr2_label_res_id \
              conect_ptnr2_label_asym_id conect_ptnr2_auth_seq_id \
              conect_ptnr2_label_atom_id conect_ptnr2_label_alt_id)
            if (bond_count[connect_bond]+0 == 0) {
              aslist[connect_bond] = "|"
              ++bond_count[connect_bond]
              conect_xtype[connect_bond] = connect_types[itarget]
              conect_symm_1[connect_bond] = "   .   "
              conect_symm_2[connect_bond] = "   .   "
              conect_role_1[connect_bond] = "."
              conect_role_2[connect_bond] = "."
              conect_epsn1[connect_bond] = ent_pol_seq_num[( \
                conect_ptnr1_label_asym_id conect_ptnr1_auth_seq_id)]
              conect_epsn2[connect_bond] = ent_pol_seq_num[( \
                conect_ptnr2_label_asym_id conect_ptnr2_auth_seq_id)]
              if (conect_epsn1[connect_bond] == "") \
                conect_epsn1[connect_bond] = ". "
              if (conect_epsn2[connect_bond] == "") \
                conect_epsn2[connect_bond] = ". "
              conect_c1_label_res_id[connect_bond] = conect_ptnr1_label_res_id
              conect_c1_label_asym_id[connect_bond] = conect_ptnr1_label_asym_id
              conect_c1_auth_seq_id[connect_bond] = conect_ptnr1_auth_seq_id
              conect_c1_label_atom_id[connect_bond] = conect_ptnr1_label_atom_id
              conect_c1_label_alt_id[connect_bond] = conect_ptnr1_label_alt_id
              conect_c2_label_res_id[connect_bond] = conect_ptnr2_label_res_id
              conect_c2_label_asym_id[connect_bond] = conect_ptnr2_label_asym_id
              conect_c2_auth_seq_id[connect_bond] = conect_ptnr2_auth_seq_id
              conect_c2_label_atom_id[connect_bond] = conect_ptnr2_label_atom_id
              conect_c2_label_alt_id[connect_bond] = conect_ptnr2_label_alt_id
              next_bond[++xconnect_flag] = connect_bond
            } # end if (bond_count[connect_bond]+0 == 0)
            if (index(aslist[connect_bond], ("|" c1 " " c2 "|")) == 0) {
              aslist[connect_bond] = (aslist[connect_bond]  c1 " " c2 "|")
            }
          } # end if (c2 != "     ")
      ...
      if (ssbond_flag > 1 ) {
        ...
        if (bond_count[connect_bond]+0 == 0) {
          warning_list[++warning_flag] = \
           ("#=# STRUCT_CONN: No connect for disulf " connect_bond " \n")
              aslist[connect_bond] = ("|. .|")
              ++bond_count[connect_bond]
              ...
              conect_ptnr2_label_asym_id = substr(connect_bond,19,1)
              ...
              if (conect_epsn2[connect_bond] == "") \
                conect_epsn2[connect_bond] = ". "
              conect_c1_label_res_id[connect_bond] = substr(connect_bond,1,3)
              conect_c1_label_asym_id[connect_bond] = conect_ptnr1_label_asym_id
              conect_c1_auth_seq_id[connect_bond] = conect_ptnr1_auth_seq_id
              conect_c1_label_atom_id[connect_bond] = substr(connect_bond,10,4)
              conect_c1_label_alt_id[connect_bond] = substr(connect_bond,14,1)
              conect_c2_label_res_id[connect_bond] =  substr(connect_bond,16,3)
              conect_c2_label_asym_id[connect_bond] = conect_ptnr2_label_asym_id
              conect_c2_auth_seq_id[connect_bond] = conect_ptnr2_auth_seq_id
              conect_c2_label_atom_id[connect_bond] = substr(connect_bond,25,4)
              conect_c2_label_alt_id[connect_bond] = substr(connect_bond,29,1)
              next_bond[++xconnect_flag] = connect_bond
        } else {
          conect_xtype[connect_bond] = "disulf"
          conect_symm_1[connect_bond] = ssbs1
      ...
      if (sltbrg_flag > 1 ) {
        ...
        connect_bond =\
          (sb_res_name_beg[i] \
          sb_chain_id_beg[i] sb_res_seq_num_beg[i] \
          sbra1 sbral1 \
          " " sb_res_name_end[i] \
          sb_chain_id_end[i] sb_res_seq_num_end[i] \
          sbra2 sbral2 )
        if ((sb_chain_id_beg[i] sb_res_seq_num_beg[i] \
          sbra1 sbral1 ) > \
          (sb_chain_id_end[i] sb_res_seq_num_end[i] \
          sbra2 sbral2)) {
          sbra1 = sb_atom_end[i]
          ...
          connect_bond =\
            (sb_res_name_end[i] \
            sb_chain_id_end[i] sb_res_seq_num_end[i] \
            sbra1 sbral1 " " sb_res_name_beg[i] \
            sb_chain_id_beg[i] sb_res_seq_num_beg[i] \
            sbra2 sbral2 )
        } # end if ((sb_chain_id_beg[i] sb_res_seq_num_beg[i] ...
        if (bond_count[connect_bond]+0 == 0) {
          if (sbrs1 == sbrs2) warning_list[++warning_flag] = \
           ("#=# STRUCT_CONN: No connect for saltbr " connect_bond " \n")
              aslist[connect_bond] = ("|. .|")
              ++bond_count[connect_bond]
          ...
          if (conect_xtype[connect_bond] != "saltbr"){
            warning_list[++warning_flag] =\
           ("#=# STRUCT_CONN: Changed bond " connect_bond " to saltbr\n")
          }
          conect_xtype[connect_bond] = "saltbr"
          conect_symm_1[connect_bond] = sbrs1
          conect_symm_2[connect_bond] = sbrs2
      ...
      if (link_flag > 1 ) {
        ...
        connect_bond =\
          (lk_res_name_beg[i] \
          lk_chain_id_beg[i] lk_res_seq_num_beg[i] \
          lnka1 lnkal1 \
          " " lk_res_name_end[i] \
          lk_chain_id_end[i] lk_res_seq_num_end[i] \
          lnka2 lnkal2 )
        if ((lk_chain_id_beg[i] lk_res_seq_num_beg[i] \
          lnka1 lnkal1 ) > \
          (lk_chain_id_end[i] lk_res_seq_num_end[i] \
          lnka2 lnkal2)) {
          lnka1 = lk_atom_end[i]
          ...
          connect_bond =\
            (lk_res_name_end[i] \
            lk_chain_id_end[i] lk_res_seq_num_end[i] \
            lnka1 lnkal1 " " lk_res_name_beg[i] \
            lk_chain_id_beg[i] lk_res_seq_num_beg[i] \
            lnka2 lnkal2 )
        } # end if ((lk_chain_id_beg[i] lk_res_seq_num_beg[i] ...
        if (bond_count[connect_bond]+0 == 0) {
          if (lnks1 == lnks2) warning_list[++warning_flag] = \
           ("#=# STRUCT_CONN: No connect for link " connect_bond " \n")
              aslist[connect_bond] = ("|. .|")
              ++bond_count[connect_bond]
      ...
      if (hydbnd_flag > 1 ) {
        ...
        hbdcrh = "hydrogen"
        if ((hbdch hbdrnh hbdalh) == \
          (hb_chain_id_beg[i] hb_res_seq_num_beg[i] hbdal1)) {
        ...
        if ((hbdch hbdrnh hbdalh) == \
          (hb_chain_id_end[i] hb_res_seq_num_end[i] hbdal2)) {
       ...
       hbdcr2s = hbdcr2
        connect_bond =\
          ...
          hbda2 hbdal2 )
        if ((hb_chain_id_beg[i] hb_res_seq_num_beg[i] \
          hbda1 hbdal1 ) > \
          (hb_chain_id_end[i] hb_res_seq_num_end[i] \
          hbda2 hbdal2)) {
          hbdcr1 = hbdcr2
          ...
          connect_bond =\
            (hb_res_name_end[i] \
            hb_chain_id_end[i] hb_res_seq_num_end[i] \
            hbda1 hbdal1 " " hb_res_name_beg[i] \
            hb_chain_id_beg[i] hb_res_seq_num_beg[i] \
            hbda2 hbdal2 )
        } # end if ((hb_chain_id_beg[i] hb_res_seq_num_beg[i] ...
        if (bond_count[connect_bond]+0 == 0) {
          if( hbds1 == hdbs2) warning_list[++warning_flag] = \
           ("#=# STRUCT_CONN: No connect for hydbnd " connect_bond " \n")
              aslist[connect_bond] = ("|. .|")
              ++bond_count[connect_bond]
          ...
          if (conect_xtype[connect_bond] != "hydrog"){
            warning_list[++warning_flag] =\
           ("#=# STRUCT_CONN: Changed bond " connect_bond " to hydrog\n")
          }
          conect_xtype[connect_bond] = "hydrog"
          conect_symm_1[connect_bond] = hbds1
          conect_symm_2[connect_bond] = hbds2
      ...
      for (i = 1; i <= xconnect_flag; ++i) {
        bond = next_bond[i]
        if (conect_role_1[bond] != "." ) {
          conect_role_1[bond] = (conect_role_1[bond] \
          "\n                                  ")
        }
        if (conect_role_2[bond] != "." ) {
          conect_role_2[bond] = (conect_role_2[bond] \
   "\n                                                               ")
        }
        dosplit(num_aslist,aslist[bond],xaslist,"|")
        for (ii = 1; ii <= num_aslist; ++ii) {
        if (xaslist[ii] != "") {
        ++icid
        dosplit(numxx,xaslist[ii],c12," ")
        printf \
          ("%6s %6s %3s %1s %5s %4s %1s %s %7s %3s %1s %5s %4s %1s %s %7s\n",\
          icid, conect_xtype[bond], conect_c1_label_res_id[bond], \
          conect_c1_label_asym_id[bond], conect_c1_auth_seq_id[bond], \
          conect_c1_label_atom_id[bond], conect_c1_label_alt_id[bond], \
          conect_role_1[bond], conect_symm_1[bond], \
          conect_c2_label_res_id[bond], conect_c2_label_asym_id[bond], \
          conect_c2_auth_seq_id[bond], conect_c2_label_atom_id[bond], \
        ...
        if ( model_flags == "yes" ) {
          ...
          if ( c12[1] != "" && c12[1] != "." ) {
            ...
            test_model = int((c12[1]+max_asn-1)/max_asn)
            ihi = model_count
            ilo = 1
            if ( test_model-ihi > 0 ) { test_model = ihi }
            while ( ihi-ilo > 0 ) {
              if (( c12[1] - model_asn_low[test_model] >= 0 ) && \
               ( c12[1] - model_asn_high[test_model] <= 0 ) ) {
                 ihi = ilo = test_model
              } else {
                if ( c12[1] - model_asn_low[test_model] < 0 ) {
                  ihi = test_model-1
                } else {
                  if (  c12[1] - model_asn_high[test_model] > 0 ) {
                    ilo = test_model+1
                  } else {
                    ihi = ilo = test_model
                  }
          ...
          if (c12[2] != "" && c12[2] != ".") {
            ...
            if ( model_bias+0 == 0 ) {
            if (( c12[2] - model_asn_low[test_model] < 0) || \
             ( c12[2] - model_asn_high[test_model] > 0 ) ) {
              warning_list[++warning_flag] = \
               ("#=# STRUCT_CONN: Bond between " \
               atom_number[c12[1]] " and " atom_number[c12[2]] \
               " crosses models \n")     
            }
            }
          }
          printf \
          (" %7s          %6s      %7s          %6s\n",\
            atom_number[c12[1]]+model_bias*max_atom_number, \
                            conect_epsn1[bond],\
            atom_number[c12[2]]+model_bias*max_atom_number, \
                            conect_epsn2[bond])
          for (ip = 1; ip < 3; ip++) {
            id_in_model = 0
            if ( root_model_rep[c12[ip]] != "" ) {
              id_in_model = atom_id_in_model[root_model_rep[c12[ip]]]
              auth_id_in_model = atom_number[root_model_rep[c12[ip]]]
            }
            if ( id_in_model+0 == 0 ) {
              id_in_model = \
              atom_id_by_type[( atom_name[c12[ip]] "|" \
                residue_name[c12[ip]] "|" \
                atom_alt_location[c12[ip]] "|" chain_id[c12[ip]] "|" \
          ...
          printf \
          ("   %5s          %6s        %5s          %6s\n",\
            c12[1], conect_epsn1[bond], c12[2], conect_epsn2[bond])
        }
        }
        }
...

  {
    if (ss_flag > 1 && ss_flag_2 == "1" ) {
      ...
      printf ("_struct_conf_type.reference\n")
        ...
        if (turn_flag > 1 && turn_flag_2 == "1")   {
        for (i=helix_flag; i < helix_flag+turn_flag-1; ++i ) {
...
#  start with helix records
        for (i=1; i < helix_flag; ++i) {
        xxx[1]=""
        dosplit(num_x,helix_comment[i],xxx," ")
        ...
        helix_res_name_end[i],\
        helix_chain_id_end[i], helix_res_seq_end[i], \
        helix_comment[i])
...
#  and now turn records
        ...
        for (i=j; i < k; ++i ) {
        xxx[1]=""
        dosplit(num_x,turn_comment[i],xxx," ")
        turn_comment[i] = xxx[1]
        for (l=2; l <= num_x; ++l) {
          turn_comment[i] = (turn_comment[i] " " xxx[l])
        }
        printf ("turn_%-3s  %6s       %3s %1s %5s %3s %1s %5s '%s'\n", \
        turn_id[i], turn_class[i], \
        turn_res_name_beg[i], turn_chain_id_beg[i], \
        turn_res_seq_beg[i], turn_res_name_end[i], \
        turn_chain_id_end[i], turn_res_seq_end[i], \
        turn_comment[i] )
    ...
    }
  {
    if (cispep_flag > 0 ) {
      ...
      printf (("_struct_mon_prot." pdb2cif_prefix "label_model_id\n"))
      ...
      for (i = 1; i <= cispep_flag; ++i) {
        ...
        cp_res_name_end[i], cp_chain_id_end[i], \
        cp_res_seq_num_end[i],  ".", epsn, \
      ...
      printf (("_struct_mon_prot_cis." pdb2cif_prefix  "label_model_id\n"))
      for (i = 1; i <= cispep_flag; ++i) {
        epsn = ent_pol_seq_num[(cp_chain_id_end[i] \
                             cp_res_seq_num_end[i])]
        if (epsn == "") epsn = ". "
      ...
      if(site_flag_1 == "2") {
        ...
        for (i=1; i < site_flag; ++i) {
          ...
          printf ( "%3s %3s %3s %1s %5s %6s . 1_555 . \n", \
            site_num, site_id[i], site_res_name_1[i], \
            site_chain_id_1[i],site_res_seq_1[i],epsn1)
          ++site_num
          if (site_res_name_2[i] != "   ") {
            epsn2 = ent_pol_seq_num[(site_chain_id_2[i] \
                                     site_res_seq_2[i])]
            if (epsn2 == "") epsn2 = ".  "
            printf ( "%3s %3s %3s %1s %5s %6s . 1_555 . \n", \
              site_num, site_id[i], site_res_name_2[i], \
              site_chain_id_2[i],site_res_seq_2[i],epsn2)
            ++site_num
          }
          if (site_res_name_3[i] != "   ") {
            epsn3 = ent_pol_seq_num[(site_chain_id_3[i] \
                                     site_res_seq_3[i])]
            if (epsn3 == "") epsn3 = ".  "
            printf ( "%3s %3s %3s %1s %5s %6s . 1_555 . \n", \
              site_num, site_id[i], site_res_name_3[i], \
              site_chain_id_3[i],site_res_seq_3[i],epsn3)
            ++site_num
          }
          if (site_res_name_4[i] != "   ") {
            epsn4 = ent_pol_seq_num[(site_chain_id_4[i] \
                                     site_res_seq_4[i])]
            if (epsn4 == "") epsn4 = ".  "
            printf ( "%3s %3s %3s %1s %5s %6s . 1_555 . \n", \
              site_num, site_id[i], site_res_name_4[i], \
              site_chain_id_4[i],site_res_seq_4[i],epsn4)
            ++site_num
          }
...
#
  if (sheet_flag > 0) {
    ...
    for (i=1; i <= sheet_flag; ++i) {
      strand = (sheet_res_name_beg[i] " " \
                sheet_chain_id_beg[i] " " \
                sheet_res_seq_beg[i]  " " \
                sheet_res_name_end[i] " " \
                sheet_chain_id_end[i] " " \
                sheet_res_seq_end[i])
      ++strand_count[strand]
      ...
      if (ordered_strand_name[strand] == "" || \
        ordered_strand_name[strand] > my_strand_name) {
        ordered_strand_name[strand] = my_strand_name
      }
      if (sheet_id_list[strand] == "") {
        sheet_id_list[strand] = sheet_id[i]
      } else {
        sheet_id_list[strand] = (sheet_id_list[strand] "|" sheet_id[i])
      }
    ...
    for (sheet_name in sheet_strands) {
    ...
    for (strand in sheet_id_list) {
      dosplit(num_sheet,sheet_id_list[strand],sheets,"|")
      ...
      for (i=2; i <= num_sheet; ++i) {
        ...
        if (first_sheet != target_sheet) {
          if (target_sheet < first_sheet) {
          ...
          while (target_sheet != "") {
            ...
            if (next_sheet > target_sheet || next_sheet == "") {
              ...
              if (sheet_size[first_sheet] < sheet_size[prev_sheet] ) {
                sheet_size[first_sheet] = sheet_size[prev_sheet]
              }
            }
          }
        }
    ...
    for (is = 1; is <= num_strand; ++is) {
      ...
      dosplit(num_x,ordered_strand_name[strand],xxx,"|")
      ordered_strand_name[strand]= \
        (sheet_root[xxx[1]] " " xxx[2] " "xxx[1])
      dosplit(numy,(xxx[2] " " xxx[1]), yyy, " ")
      ...
      while (reloop == "yes") {
        if (ordered_strand_name[strand] < \
            ordered_strand_name[next_strand] || \
            next_strand == "") {
    ...
    for (is = 1; is <= num_strand; ++is) {
      strand = strand_list[is]
      dosplit(my_num,sheet_id_list[strand],my_sid,"|")
      i = my_sid[1]
    ...
    for (ip = 1; ip <= num_pair; ++ip) {
      my_pair = pair_list[ip]
      dosplit(my_p_num,my_pair,my_strands,"|")
      strand = my_strands[1]
    ...
    for (ip=1; ip <= num_pair; ++ip) {
      ...
      if (sheet_strand_no[i] > 1) {
        ...
        p_strand_name = strand_name[p_strand]
        printf("%3s %7s %7s +1 %s\n",\
          sheet_root[sheet_id[i]],p_strand_name,my_strand_name,sheet_sense[i])
      }
    }
    ...
    for (is = 1; is <= num_strand; ++is ){
      strand = strand_list[is]
      dosplit(my_stand_num,strand,my_strand," ")
      for (k = my_strand[3]; k <= my_strand[6]; k++) {
        strand_atl[(my_strand[2] " " k)] = strand_name[strand]
      }
      dosplit(my_num,sheet_id_list[strand],my_sid,"|")
      i = my_sid[1]
      my_sheet_name = sheet_root[i]
  ...
  for (i=1; i < atom_flag; i++) {
    ...
    if (cur_model_count > 1) break
    if (atom_ent_seq_num[i] != ".") {
      seq_key = (residue_name[i] "|" chain_id[i] "|" residue_seq_number[i] "|" cur_model)
      if (poly_seq_al[(chain_id[i] " " atom_ent_seq_num[i])] == "") {
        poly_seq_al[(chain_id[i] " " atom_ent_seq_num[i])] = i
        poly_seq_al_key[(chain_id[i] " " atom_ent_seq_num[i])] = seq_key
      } else {
        if (poly_seq_al_key[(chain_id[i] " " atom_ent_seq_num[i])] != seq_key) {
  ...
  if (num_poly_ents > 0) {
    ...
    for (i=1; i <= num_poly_ents; ++i) {
      for (seq_number = seq_start_num[entity_seq_num[entities[i]]]; \
        ...
        strand_info = "."
        if (strand_atl[(entities[i] " " seq_number)] != "") {
           strand_info = strand_atl[(entities[i] " " seq_number)]
        }
        if (iatdat == "") {
          printf (" %1s %5d %4s %3s    .  %s\n",\
          entities[i], entity_seq_num[entities[i]], \
          seq_number, entity_poly_seq[seq_number], strand_info)
        } else {
          dosplit(numiatdat,iatdat,iatdats,"|")
          for (k=1; k <= numiatdat; k++) {
            printf (" %1s %5d %4s %3s %5s %s\n",\
              entities[i], entity_seq_num[entities[i]], \
              seq_number, entity_poly_seq[seq_number], \
  ...
  #
  if (model_flags == "yes" || seqadv_flag+0 > 0 || \
    ...
    printf ("   %s\n", head_PDB_code)
    printf (("  '_atom_site." pdb2cif_prefix "auth_id_in_model'\n"))
    printf ("      'temporary place holder for NMR author id in model'\n")
    printf ("      no\n\n")
    ...
    printf ("   %s\n", head_PDB_code)
    printf (("  '_struct_conn." pdb2cif_prefix "ptnr1_atom_site_id'\n"))
    printf ("      '_atom_site.id of partner 1 of structure connection'\n")
    printf ("      no\n\n")
    printf ("   %s\n", head_PDB_code)
    printf (("  '_struct_conn." pdb2cif_prefix "ptnr2_atom_site_id'\n"))
    printf ("      '_atom_site.id of partner 2 of structure connection'\n")
    printf ("      no\n\n")
    if ( model_flags == "yes" ) {
      warning_list[++warning_flag] = \
        ("#=# PUBL_MANUSCRIPT_INCL: Tokens _struct_conn.\n" \
    ...
    warning_list[++warning_flag] = \
      ...
      "db_seq_num used\n")
    printf ("   %s\n", head_PDB_code)
    printf (("  '_struct_ref_seq_dif." pdb2cif_prefix "db_seq_num'\n"))
...
ifdef(\036NOFUNCS\037,,
function \036\036squeezname\037\037( str, temp_a_name) {
squeezname( str, temp_a_name)
})
ifdef(\036NOFUNCS\037,,
function \036\036squeeztype\037\037( str, temp_a_type) {
squeeztype( str, temp_a_type)
})
ifdef(\036NOFUNCS\037,,
function \036\036typeset\037\037( str,  lostr,lstr,mystr,pchar,i) {
typeset( str,  lostr,lstr,mystr,pchar,i)
})
ifdef(\036NOFUNCS\037,,
function \036\036nameset\037\037( str,  lostr,lstr,mystr,pchar,i) {
nameset( str,  lostr,lstr,mystr,pchar,i)
})
ifdef(\036NOFUNCS\037,,
function \036\036seq_comp\037\037(seqal,seqsr,seq_match,\
 lsal,lssr,lm,isha,isr,ksa,nm) {
seq_comp(seqal,seqsr,seq_match, lsal,lssr,lm,isa,isr,ksa)
})
